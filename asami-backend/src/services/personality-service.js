...
async function getCognitiveProfiles(simulationId, entityIds=[]) {
  ...
  const [preferences]=await pool.query(`SELECT entityId,id,targetType,targetEntityId,preferenceValue,strength,confidence,updatedAt FROM (SELECT BIN_TO_UUID(entity_id) AS entityId,BIN_TO_UUID(id) AS id,target_type AS targetType,BIN_TO_UUID(target_entity_id) AS targetEntityId,preference_value AS preferenceValue,strength,confidence,updated_simulation_at AS updatedAt,ROW_NUMBER() OVER(PARTITION BY entity_id ORDER BY strength DESC,confidence DESC,updated_simulation_at DESC) AS rn FROM preferences WHERE simulation_id=UUID_TO_BIN(?) AND entity_id IN (${placeholders})) ranked WHERE rn<=24 ORDER BY entityId,rn`,[simulationId,...ids]);
  const [beliefs]=await pool.query(`SELECT entityId,id,subjectEntityId,predicate,objectValue,confidence,importance,status,updatedAt FROM (SELECT BIN_TO_UUID(entity_id) AS entityId,BIN_TO_UUID(id) AS id,BIN_TO_UUID(subject_entity_id) AS subjectEntityId,predicate,object_value AS objectValue,confidence,importance,status,updated_simulation_at AS updatedAt,ROW_NUMBER() OVER(PARTITION BY entity_id ORDER BY importance DESC,confidence DESC,updated_simulation_at DESC) AS rn FROM beliefs WHERE simulation_id=UUID_TO_BIN(?) AND entity_id IN (${placeholders}) AND status IN ('ACTIVE','REVISED')) ranked WHERE rn<=24 ORDER BY entityId,rn`,[simulationId,...ids]);
  const [knowledge]=await pool.query(`SELECT entityId,id,knowledgeType,subjectEntityId,predicate,objectEntityId,content,confidence,importance,learnedAt FROM (SELECT BIN_TO_UUID(ek.entity_id) AS entityId,BIN_TO_UUID(ek.knowledge_item_id) AS id,ki.knowledge_type AS knowledgeType,BIN_TO_UUID(ki.subject_entity_id) AS subjectEntityId,ki.predicate,BIN_TO_UUID(ki.object_entity_id) AS objectEntityId,ki.content,ek.confidence,ek.importance,ek.learned_simulation_at AS learnedAt,ROW_NUMBER() OVER(PARTITION BY ek.entity_id ORDER BY ek.importance DESC,ek.confidence DESC,ek.last_reinforced_at DESC,ek.learned_simulation_at DESC) AS rn FROM entity_knowledge ek JOIN knowledge_items ki ON ki.id=ek.knowledge_item_id WHERE ek.simulation_id=UUID_TO_BIN(?) AND ek.entity_id IN (${placeholders}) AND ek.status='ACTIVE') ranked WHERE rn<=24 ORDER BY entityId,rn`,[simulationId,...ids]);
  const [habits]=await pool.query(`SELECT entityId,id,name,description,strength,frequency,triggerDefinition,actionDefinition,status,updatedAt FROM (SELECT BIN_TO_UUID(entity_id) AS entityId,BIN_TO_UUID(id) AS id,name,description,strength,frequency,trigger_definition AS triggerDefinition,action_definition AS actionDefinition,status,updated_simulation_at AS updatedAt,ROW_NUMBER() OVER(PARTITION BY entity_id ORDER BY strength DESC,updated_simulation_at DESC) AS rn FROM habits WHERE entity_id IN (${placeholders}) AND status IN ('ACTIVE','WEAKENING')) ranked WHERE rn<=16 ORDER BY entityId,rn`,ids);
  const [plans]=await pool.query(`SELECT id,entityId,goalId,title,status,strategy,createdAt FROM (SELECT BIN_TO_UUID(id) AS id,BIN_TO_UUID(entity_id) AS entityId,BIN_TO_UUID(goal_id) AS goalId,title,status,strategy,created_simulation_at AS createdAt,ROW_NUMBER() OVER(PARTITION BY entity_id ORDER BY created_simulation_at DESC) AS rn FROM plans WHERE simulation_id=UUID_TO_BIN(?) AND entity_id IN (${placeholders}) AND status IN ('DRAFT','ACTIVE','PAUSED')) ranked WHERE rn<=8 ORDER BY entityId,rn`,[simulationId,...ids]);
