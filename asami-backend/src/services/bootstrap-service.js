...
const EMOTIONS = [
  ...
  ["00000000-0000-4000-8002-000000000006", "FRUSTRATION", "Frustration", 0.2],
  ["00000000-0000-4000-8002-000000000007", "EXCITEMENT", "Excitement", 0.25],
  ["00000000-0000-4000-8002-000000000008", "CALM", "Calm", 0.1],
  ["00000000-0000-4000-8002-000000000009", "DISGUST", "Disgust", 0.15],
  ["00000000-0000-4000-8002-000000000010", "SHAME", "Shame", 0.1]
];
...
const TRAITS = [
  ...
  ["00000000-0000-4000-8003-000000000007", "IMPULSIVITY", "Impulsivity", 0.1, 0.9],
  ...
  ["00000000-0000-4000-8003-000000000012", "RISK_TAKING", "Risk Taking", 0.1, 0.9],
  ["00000000-0000-4000-8003-000000000013", "SOCIABILITY", "Sociability", 0.09, 0.9],
  ["00000000-0000-4000-8003-000000000014", "INDEPENDENCE", "Independence", 0.1, 1],
  ["00000000-0000-4000-8003-000000000015", "DISCIPLINE", "Discipline", 0.08, 1],
  ["00000000-0000-4000-8003-000000000016", "CREATIVITY", "Creativity", 0.1, 1.1]
];

const SKILLS = [
  ["00000000-0000-4000-8004-000000000001", "READING", "Reading", "ACADEMIC"],
  ["00000000-0000-4000-8004-000000000002", "WRITING", "Writing", "ACADEMIC"],
  ["00000000-0000-4000-8004-000000000003", "DRAWING", "Drawing", "CREATIVE"],
  ["00000000-0000-4000-8004-000000000004", "COOKING", "Cooking", "PRACTICAL"],
  ["00000000-0000-4000-8004-000000000005", "COMMUNICATION", "Communication", "SOCIAL"],
  ["00000000-0000-4000-8004-000000000006", "PROBLEM_SOLVING", "Problem Solving", "COGNITIVE"],
  ["00000000-0000-4000-8004-000000000007", "MEMORY", "Memory", "COGNITIVE"],
  ["00000000-0000-4000-8004-000000000008", "MATH", "Mathematics", "ACADEMIC"],
  ["00000000-0000-4000-8004-000000000009", "MUSIC", "Music", "CREATIVE"],
  ["00000000-0000-4000-8004-000000000010", "SPORTS", "Sports", "PHYSICAL"],
  ["00000000-0000-4000-8004-000000000011", "SELF_CARE", "Self Care", "PRACTICAL"],
  ["00000000-0000-4000-8004-000000000012", "NAVIGATION", "Navigation", "PRACTICAL"]
];

const ACTIVITIES = [
  ["SLEEPING", "Sleeping", "BIOLOGICAL"], ["EATING", "Eating", "BIOLOGICAL"],
  ["DRINKING", "Drinking", "BIOLOGICAL"], ["SCHOOL", "School", "EDUCATION"],
  ["STUDYING", "Studying", "EDUCATION"], ["PLAYING", "Playing", "LEISURE"],
  ["WALKING", "Walking", "MOVEMENT"], ["TALKING", "Talking", "SOCIAL"],
  ["TRAVELLING", "Travelling", "MOVEMENT"], ["WATCHING", "Watching", "LEISURE"],
  ["READING", "Reading", "LEISURE"], ["WORKING", "Working", "WORK"],
  ["RESTING", "Resting", "BIOLOGICAL"], ["EXPLORING", "Exploring", "EXPLORATION"]
];

const EVENT_TYPES = [
  ["PERSONAL", "Personal Event", "PERSONAL"], ["SOCIAL", "Social Event", "SOCIAL"],
  ["FAMILY", "Family Event", "FAMILY"], ["SCHOOL", "School Event", "SCHOOL"],
  ["ENVIRONMENTAL", "Environmental Event", "ENVIRONMENTAL"], ["RELATIONSHIP", "Relationship Event", "RELATIONSHIP"],
  ["INTERNAL", "Internal Event", "INTERNAL"], ["RANDOM", "Random Event", "RANDOM"],
  ["COMMUNICATION", "Communication Event", "SOCIAL"]
];

const RELATIONSHIPS = [
  ["PARENT", "Parent", 0], ["CHILD", "Child", 0], ["SIBLING", "Sibling", 1],
  ["FRIEND", "Friend", 1], ["ACQUAINTANCE", "Acquaintance", 1], ["TEACHER", "Teacher", 0],
  ["STUDENT", "Student", 0], ["CAREGIVER", "Caregiver", 0], ["EMPLOYER", "Employer", 0], ["COLLEAGUE", "Colleague", 1]
];
...
async function ensureEntityTypes() {
  for (const [id, code, name, category] of ENTITY_TYPES) {
    await pool.query(`
      INSERT INTO entity_types(id,code,name,category,schema_definition,active)
...
async function ensureNeeds() {
  for (const [id, code, name, min, max, def, decay, recovery, priority] of NEEDS) {
    await pool.query(`
...
async function ensureTraits() {
  for (const [id, code, name, volatility, developmentWeight] of TRAITS) {
    await pool.query(`
...
async function bootstrapCoreDefinitions() {
  ...
  for (let i = 0; i < ACTIVITIES.length; i += 1) {
    const [code, name, category] = ACTIVITIES[i];
    const id = `00000000-0000-4000-8005-${String(i + 1).padStart(12, "0")}`;
    await pool.query(`
      INSERT INTO activity_types(id,code,name,category,parameters,active)
      VALUES(UUID_TO_BIN(?),?,?,?,NULL,1)
      ON DUPLICATE KEY UPDATE name=VALUES(name),category=VALUES(category),active=1
    `, [id, code, name, category]);
  }
  for (let i = 0; i < EVENT_TYPES.length; i += 1) {
    const [code, name, category] = EVENT_TYPES[i];
    const id = `00000000-0000-4000-8006-${String(i + 1).padStart(12, "0")}`;
    await pool.query(`
      INSERT INTO event_types(id,code,name,category,configuration,active)
      VALUES(UUID_TO_BIN(?),?,?,?,NULL,1)
      ON DUPLICATE KEY UPDATE name=VALUES(name),category=VALUES(category),active=1
    `, [id, code, name, category]);
  }
  for (let i = 0; i < RELATIONSHIPS.length; i += 1) {
    const [code, name, symmetric] = RELATIONSHIPS[i];
    const id = `00000000-0000-4000-8007-${String(i + 1).padStart(12, "0")}`;
    await pool.query(`
      INSERT INTO relationship_types(id,code,name,symmetric,configuration,active)
      VALUES(UUID_TO_BIN(?),?,?,?,NULL,1)
      ON DUPLICATE KEY UPDATE name=VALUES(name),symmetric=VALUES(symmetric),active=1
    `, [id, code, name, symmetric]);
  }
  for (let i = 0; i < DEVELOPMENT_STAGES.length; i += 1) {
    const [code, name, minAge, maxAge] = DEVELOPMENT_STAGES[i];
    const id = `00000000-0000-4000-8008-${String(i + 1).padStart(12, "0")}`;
    await pool.query(`
      INSERT INTO development_stages(id,code,name,min_age_days,max_age_days,configuration,active)
      VALUES(UUID_TO_BIN(?),?,?,?, ?,NULL,1)
      ON DUPLICATE KEY UPDATE name=VALUES(name),min_age_days=VALUES(min_age_days),max_age_days=VALUES(max_age_days),active=1
    `, [id, code, name, minAge, maxAge]);
  }
