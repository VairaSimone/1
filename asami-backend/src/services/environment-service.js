...
async function isVitalEnvironmentalEventDue(simulationId,simulationTime){
  const locations=(await loadLocations(simulationId))
    .filter(location=>Array.isArray(EVENTS[location.locationType])&&EVENTS[location.locationType].some(item=>VITAL_EVENT_CODES.has(item[0])));
  if(!locations.length)return false;
  const selected=await selectFairEnvironmentalLocation(simulationId,locations,simulationTime);
  return Boolean(
    selected &&
    selected.fairness.ageHours>=FAIR_EVENT_MAX_GAP_HOURS &&
    selected.fairness.deficit>0 &&
    selected.fairness.vitalBoost>0
  );
}
...
module.exports={updateEnvironmentState,generateEnvironmentalEvent,processWorldEffects,isVitalEnvironmentalEventDue,eventProbability:(elapsedSimulationMinutes,eventsPerSimulationHour=1)=>1-Math.exp(-(Math.max(0,Number(elapsedSimulationMinutes)||0)*Math.max(0,Number(eventsPerSimulationHour)||0))/60),WEATHER,EVENTS};