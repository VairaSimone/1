...
const ACTION_PRESSURE_FLOORS={
  TALKING:{SOCIAL_NEED:0.18,BELONGING:0.15}, PLAYING:{FUN:0.18,SOCIAL_NEED:0.10},
  STUDYING:{ACHIEVEMENT:0.15,CURIOSITY:0.15}, READING:{CURIOSITY:0.15,ACHIEVEMENT:0.12},
  EXPLORING:{CURIOSITY:0.15}, WORKING:{ACHIEVEMENT:0.15}, WALKING:{FUN:0.15,CURIOSITY:0.10}, WATCHING:{FUN:0.15}
};
...
async function updateNeeds(entityId,simulationTime,deltaHours,causeEventId=null,causeActionId=null,activeActionType=null){
  const rows=await readNeeds(entityId),changes=[];
  const hours=Math.min(Math.max(Number(deltaHours)||0,0),6);
  const gains={
    SLEEPING:{SLEEPINESS:-1.8,ENERGY:0.9,COMFORT:0.35,SAFETY:0.08}, EATING:{HUNGER:-2.4,ENERGY:0.1,COMFORT:0.15,SAFETY:0.03},
    DRINKING:{THIRST:-3,ENERGY:0.1,SAFETY:0.02}, TALKING:{SOCIAL_NEED:-0.45,BELONGING:-0.18},
    PLAYING:{FUN:-0.5,SOCIAL_NEED:-0.12,COMFORT:0.04}, RESTING:{ENERGY:0.6,COMFORT:0.5,SLEEPINESS:-0.2},
    STUDYING:{ACHIEVEMENT:-0.8,CURIOSITY:-0.5,ENERGY:-0.15,FUN:-0.1,COMFORT:-0.03}, READING:{CURIOSITY:-0.4,ACHIEVEMENT:-0.3,FUN:0.1,COMFORT:0.03},
    EXPLORING:{CURIOSITY:-1.2,FUN:-0.5,ENERGY:-0.15,COMFORT:-0.05,SAFETY:-0.02}, WALKING:{FUN:-0.25,ENERGY:-0.08,COMFORT:0.02,SAFETY:0.01},
    WORKING:{ACHIEVEMENT:-0.7,ENERGY:-0.2,FUN:-0.1,COMFORT:-0.04,SAFETY:0.005}, WATCHING:{FUN:-1.1,ENERGY:0.05,COMFORT:0.06,SAFETY:0.02},
    SCHOOL:{ACHIEVEMENT:-0.6,CURIOSITY:-0.4,ENERGY:-0.12,FUN:-0.05,COMFORT:-0.03,SAFETY:0.01}
  };
  for(const r of rows){
    const decayRate=Math.max(0,Number(r.decayRate)||0),recoveryRate=Math.max(0,Number(r.recoveryRate)||0); let delta;
    ...
    if(activeActionType){
      ...
      const rawGain=gain*hours;
      const floor=ACTION_PRESSURE_FLOORS[activeActionType]?.[r.code];
      if(PRESSURE_NEEDS.has(r.code)&&rawGain<0&&floor!==undefined){
        const satisfiable=Math.max(0,Number(r.value)-floor); delta+=Math.max(rawGain,-satisfiable);
      } else if(PRESSURE_NEEDS.has(r.code)&&rawGain<0) delta+=Math.max(rawGain,-Number(r.value)*0.25);
      else delta+=rawGain;
    }
...
function emotionAppraisal(actionType,needs){
  const codes=["JOY","SADNESS","ANGER","FEAR","ANXIETY","FRUSTRATION","EXCITEMENT","CALM","DISGUST","SHAME"],deltas=Object.fromEntries(codes.map(c=>[c,0]));
  const add=(code,value)=>{deltas[code]+=value;};
  const actionEffects={TALKING:{JOY:0.035,CALM:0.02,EXCITEMENT:0.012,ANXIETY:-0.008,SADNESS:-0.008},PLAYING:{JOY:0.045,EXCITEMENT:0.035,CALM:0.008,FRUSTRATION:-0.02,SADNESS:-0.01},EATING:{JOY:0.02,CALM:0.025,FRUSTRATION:-0.02,ANXIETY:-0.01},DRINKING:{CALM:0.025,JOY:0.012,ANXIETY:-0.012},SLEEPING:{CALM:0.04,JOY:0.015,ANXIETY:-0.02,FRUSTRATION:-0.02},RESTING:{CALM:0.03,JOY:0.012,ANXIETY:-0.015,FRUSTRATION:-0.015},STUDYING:{EXCITEMENT:0.012,JOY:0.012,FRUSTRATION:0.008,CALM:0.008},READING:{JOY:0.018,CALM:0.018,EXCITEMENT:0.01},EXPLORING:{EXCITEMENT:0.05,JOY:0.025,FEAR:0.018,ANXIETY:0.01},WALKING:{CALM:0.025,JOY:0.012,ANXIETY:-0.01},WORKING:{FRUSTRATION:0.012},SCHOOL:{FRUSTRATION:0.008,EXCITEMENT:0.008,ANXIETY:0.004},WATCHING:{JOY:0.022,CALM:0.015,EXCITEMENT:0.018}};
  for(const [code,value] of Object.entries(actionEffects[actionType]||{})) add(code,value);
  const p=Object.fromEntries(needs.map(n=>[n.code,clamp(n.new)]));
  const social=p.SOCIAL_NEED||0,belonging=p.BELONGING||0,hunger=p.HUNGER||0,thirst=p.THIRST||0,sleepiness=p.SLEEPINESS||0,fun=p.FUN||0,achievement=p.ACHIEVEMENT||0,curiosity=p.CURIOSITY||0,safety=p.SAFETY??1,energy=p.ENERGY??1;
  if(social>0.65){add("SADNESS",social*0.012);add("ANXIETY",social*0.008);add("FRUSTRATION",social*0.01);} if(belonging>0.65){add("SADNESS",belonging*0.015);add("ANXIETY",belonging*0.01);} if(hunger>0.7){add("FRUSTRATION",hunger*0.018);add("ANGER",hunger*0.01);} if(thirst>0.7){add("FRUSTRATION",thirst*0.02);add("ANXIETY",thirst*0.008);} if(sleepiness>0.7){add("FRUSTRATION",sleepiness*0.012);add("SADNESS",sleepiness*0.008);} if(fun>0.75){add("SADNESS",fun*0.01);add("FRUSTRATION",fun*0.008);} if(achievement>0.75)add("FRUSTRATION",achievement*0.01); if(curiosity>0.75)add("EXCITEMENT",curiosity*0.012); if(safety<0.45){add("FEAR",(0.45-safety)*0.06);add("ANXIETY",(0.45-safety)*0.04);} if(energy<0.3){add("FRUSTRATION",(0.3-energy)*0.04);add("SADNESS",(0.3-energy)*0.025);}
  return deltas;
}

async function applyEmotions(entityId,simulationTime,changes,causeEventId=null,causeActionId=null,actionType=null,deltaHours=0){
...
async function getTraits(entityId){
  const [rows]=await pool.query(`SELECT BIN_TO_UUID(etc.trait_id) AS traitId,td.code,etc.value,etc.version,td.volatility,td.development_weight AS developmentWeight FROM entity_traits_current etc JOIN trait_definitions td ON td.id=etc.trait_id WHERE etc.entity_id=UUID_TO_BIN(?) AND td.active=1`,[entityId]); return rows;
}
async function developTraits(entityId,simulationTime,signals,causeEventId=null,causeActionId=null){
  const traits=await getTraits(entityId),out=[]; for(const t of traits){const signal=Number(signals[t.code]||0),delta=Math.max(-0.01,Math.min(0.01,signal*Number(t.developmentWeight)*0.001));if(Math.abs(delta)<0.000001)continue;const old=round5(t.value),next=round5(clamp(old+delta)),historyDelta=round5(next-old);const [r]=await pool.query(`UPDATE entity_traits_current SET value=?,updated_simulation_at=?,version=version+1 WHERE entity_id=UUID_TO_BIN(?) AND trait_id=UUID_TO_BIN(?) AND version=?`,[next,simulationTime,entityId,t.traitId,t.version]);if(!r.affectedRows)continue;await pool.query(`INSERT INTO entity_trait_history(id,entity_id,trait_id,old_value,new_value,delta,changed_simulation_at,cause_event_id,cause_action_id,change_reason) VALUES(UUID_TO_BIN(?),UUID_TO_BIN(?),UUID_TO_BIN(?),?,?,?,?,UUID_TO_BIN(?),UUID_TO_BIN(?),?)`,[uuid(),entityId,t.traitId,old,next,historyDelta,simulationTime,causeEventId,causeActionId,"behavioral reinforcement"]);out.push({code:t.code,old,next,delta:historyDelta});} return out;
}
module.exports={ensureEntityState,readNeeds,updateNeeds,applyEmotions,getTraits,developTraits};
