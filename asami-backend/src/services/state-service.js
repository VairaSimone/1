...
async function applyEmotions(entityId,simulationTime,changes,causeEventId=null,causeActionId=null,actionType=null,deltaHours=0,appraisalContext=null){
  ...
  for(const row of rows){
    const oldIntensity=round5(row.intensity),baseline=clamp(row.defaultValue),decayCoefficient=Math.max(0,Number(row.decayRate)||0)*0.5,relaxation=hours>0?1-Math.exp(-decayCoefficient*hours):0,pressureValues=(changes||[]).filter(change=>PRESSURE_NEEDS.has(String(change.code||"").toUpperCase())).map(change=>clamp(change.new??change.old??0)),unresolvedPressure=pressureValues.length?pressureValues.reduce((sum,value)=>sum+value,0)/pressureValues.length:0,recoveryFactor=1-clamp(unresolvedPressure),recoveryRelaxation=Math.min(.70,relaxation*(1+.90*recoveryFactor)),successOutcome=appraisalContext?.event&&String(appraisalContext?.outcome||"").toUpperCase()==="SUCCESS",eventHomeostasis=appraisalContext?.event?(successOutcome?.22:.10):0,passiveDecay=(baseline-oldIntensity)*Math.max(recoveryRelaxation,eventHomeostasis),rawAppraisal=Number(appraisal[row.code]||0),positiveEmotion=new Set(["JOY","CALM","EXCITEMENT"]).has(row.code),stimulusScale=appraisalContext?.event?(positiveEmotion?Math.max(.06,1-.90*oldIntensity):Math.max(.35,1-.25*oldIntensity)):hours,next=round5(clamp(oldIntensity+passiveDecay+rawAppraisal*stimulusScale)),historyDelta=round5(next-oldIntensity);
    if(Math.abs(historyDelta)<0.000001)continue;
...
module.exports={ensureEntityState,readNeeds,updateNeeds,applyEmotions,getTraits,developTraits,emotionAppraisal,traitBehaviorWeights,safetyContextDelta,persistNeedTransition,accumulateNeedHistory,accumulateEmotionHistory,flushPendingNeedHistory,flushPendingEmotionHistory,withEntityStateLock};
