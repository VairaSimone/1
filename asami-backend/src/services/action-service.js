...
const SKILL_LEARNING_PROFILES=Object.freeze({
  READING:{baseGain:.0024,minIntervalMinutes:90},
  WRITING:{baseGain:.0022,minIntervalMinutes:90},
  COMMUNICATION:{baseGain:.0020,minIntervalMinutes:90},
  NAVIGATION:{baseGain:.0014,minIntervalMinutes:120},
  SPORTS:{baseGain:.0025,minIntervalMinutes:120},
  COOKING:{baseGain:.0020,minIntervalMinutes:120}
});
const SKILL_ACTION_MULTIPLIERS=Object.freeze({WALKING:.65,EXPLORING:1,READING:1,STUDYING:1,TALKING:1,PLAYING:1,EATING:1});
function calculateSkillLearningGain({skill,actionType,proficiency=0,lastUsedSimulationAt=null,simulationTime}={}) {
  const profile=SKILL_LEARNING_PROFILES[String(skill||"").toUpperCase()];
  if(!profile)return 0;
  const current=Math.max(0,Math.min(1,Number(proficiency)||0));
  if(current>=.9999)return 0;
  const now=new Date(simulationTime).getTime();
  if(!Number.isFinite(now))return 0;
  if(lastUsedSimulationAt){
    const last=new Date(lastUsedSimulationAt).getTime();
    if(Number.isFinite(last) && now-last < profile.minIntervalMinutes*60000)return 0;
  }
  const saturation=.35+.65*(1-current);
  const actionMultiplier=Number(SKILL_ACTION_MULTIPLIERS[String(actionType||"").toUpperCase()]||1);
  return Math.min((1-current),profile.baseGain*saturation*actionMultiplier);
}
...
async function loadLocationGraph(simulationId){const[rows]=await pool.query(`SELECT BIN_TO_UUID(e.id) AS locationId,address_data AS addressData,latitude,longitude,location_type AS locationType FROM locations l JOIN entities e ON e.id=l.entity_id WHERE l.simulation_id=UUID_TO_BIN(?) AND e.simulation_id=UUID_TO_BIN(?) AND e.status='ACTIVE'`,[simulationId,simulationId]);return rows.map(row=>({locationId:row.locationId,data:parseJson(row.addressData),latitude:coordinate(row.latitude),longitude:coordinate(row.longitude),locationType:row.locationType}));}
function resolveConnection(byId,byCode,value){if(value===null||value===undefined)return null;const key=String(value);return byId.get(key)||byCode.get(key.toUpperCase())||null;}
function shortestRoute(locations,originId,targetId){if(!originId)return null;const byId=new Map(locations.map(location=>[String(location.locationId),location])),byCode=new Map(locations.filter(location=>location.data?.worldCode).map(location=>[String(location.data.worldCode).toUpperCase(),location])),origin=byId.get(String(originId));if(!origin)return null;if(!targetId||String(targetId)===String(originId))return{path:[originId],distanceMeters:0};if(!byId.has(String(targetId)))return null;const distances=new Map(),previous=new Map(),unvisited=new Set(locations.map(location=>location.locationId));for(const id of unvisited)distances.set(id,Infinity);distances.set(originId,0);while(unvisited.size){let currentId=null,currentDistance=Infinity;for(const id of unvisited){const distance=distances.get(id);if(distance<currentDistance){currentDistance=distance;currentId=id;}}if(currentId===null||currentDistance===Infinity)break;unvisited.delete(currentId);if(String(currentId)===String(targetId))break;const current=byId.get(String(currentId));for(const connection of Array.isArray(current?.data?.connections)?current.data.connections:[]){const next=resolveConnection(byId,byCode,connection);if(!next||!unvisited.has(next.locationId))continue;const rawEdge=haversineMeters(current,next),edge=Number.isFinite(rawEdge)?Math.max(5,rawEdge*ROAD_FACTOR):5,candidate=currentDistance+edge;if(candidate<distances.get(next.locationId)){distances.set(next.locationId,candidate);previous.set(next.locationId,currentId);}}}if(!Number.isFinite(distances.get(targetId)))return null;const path=[];let cursor=targetId;while(cursor!==undefined&&cursor!==null){path.unshift(cursor);if(String(cursor)===String(originId))break;cursor=previous.get(cursor);}return String(path[0])===String(originId)?{path,distanceMeters:distances.get(targetId)}:null;}
function nextHop(locations,originId,targetId=null){if(!originId)return null;if(targetId)return shortestRoute(locations,originId,targetId)?.path[1]||null;const byId=new Map(locations.map(location=>[String(location.locationId),location])),byCode=new Map(locations.filter(location=>location.data?.worldCode).map(location=>[String(location.data.worldCode).toUpperCase(),location])),origin=byId.get(String(originId));if(!origin)return null;for(const connection of Array.isArray(origin.data?.connections)?origin.data.connections:[]){const next=resolveConnection(byId,byCode,connection);if(next&&String(next.locationId)!==String(originId))return next.locationId;}return null;}
async function chooseDestination(simulationId,entityId,originId,targetId=null){if(!originId)return null;return nextHop(await loadLocationGraph(simulationId),originId,targetId);}
...
async function completeMovement({simulationId,entityId,destination,movementId=null,simulationTime,db=pool}){const query=movementId?`SELECT BIN_TO_UUID(id) AS id,BIN_TO_UUID(origin_location_id) AS originLocationId,BIN_TO_UUID(destination_location_id) AS destinationLocationId,version FROM movements WHERE id=UUID_TO_BIN(?) AND entity_id=UUID_TO_BIN(?) AND status='ACTIVE' LIMIT 1`:`SELECT BIN_TO_UUID(id) AS id,BIN_TO_UUID(origin_location_id) AS originLocationId,BIN_TO_UUID(destination_location_id) AS destinationLocationId,version FROM movements WHERE simulation_id=UUID_TO_BIN(?) AND entity_id=UUID_TO_BIN(?) AND destination_location_id=UUID_TO_BIN(?) AND status='ACTIVE' ORDER BY started_simulation_at DESC LIMIT 1`;const params=movementId?[movementId,entityId]:[entityId,destination],[rows]=await db.query(query,params);if(!rows.length)return false;const movement=rows[0],resolvedDestination=movement.destinationLocationId||destination;if(!resolvedDestination)return false;const[updated]=await db.query(`UPDATE movements SET status='COMPLETED',actual_arrival_simulation_at=?,version=version+1 WHERE id=UUID_TO_BIN(?) AND status='ACTIVE' AND version=?`,[simulationTime,movement.id,movement.version]);if(!updated.affectedRows)return false;await db.query(`INSERT INTO entity_location_history(id,entity_id,location_id,entered_simulation_at,reason,source_event_id) VALUES(UUID_TO_BIN(?),UUID_TO_BIN(?),UUID_TO_BIN(?),UUID_TO_BIN(?),?,'AUTONOMOUS',NULL)`,[uuid(),entityId,resolvedDestination,simulationTime]);await db.query(`UPDATE entity_location_history SET exited_simulation_at=? WHERE entity_id=UUID_TO_BIN(?) AND exited_simulation_at IS NULL AND entered_simulation_at<?`,[simulationTime,entityId,simulationTime]);await db.query(`INSERT INTO entity_locations_current(entity_id,simulation_id,location_id,since_simulation_at,reason,version) VALUES(UUID_TO_BIN(?),UUID_TO_BIN(?),UUID_TO_BIN(?),?,'AUTONOMOUS',1) ON DUPLICATE KEY UPDATE location_id=VALUES(location_id),since_simulation_at=VALUES(since_simulation_at),reason=VALUES(reason),version=version+1`,[entityId,simulationId,resolvedDestination,simulationTime]);return true;}
...
async function learnFromAction(entityId,actionType,simulationTime){
  ...
  if(!skill)return;
  const[rows]=await pool.query(`SELECT BIN_TO_UUID(es.skill_id) AS skillId,es.proficiency,es.confidence,es.last_used_simulation_at AS lastUsedSimulationAt,es.version FROM entity_skills es JOIN skill_definitions sd ON sd.id=es.skill_id WHERE es.entity_id=UUID_TO_BIN(?) AND sd.code=? LIMIT 1`,[entityId,skill]);
  if(!rows.length)return;
  const current=rows[0],gain=calculateSkillLearningGain({skill,actionType:normalizedAction,proficiency:current.proficiency,lastUsedSimulationAt:current.lastUsedSimulationAt,simulationTime});
  if(gain<=0)return;
