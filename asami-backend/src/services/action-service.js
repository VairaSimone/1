...
async function resolveActivityType(code) {
  const [rows]=await pool.query("SELECT id FROM activity_types WHERE code=? AND active=1 LIMIT 1",[code]);
...
async function chooseDestination(simulationId, entityId, originId) {
  const [rows]=await pool.query(`
    ...
    WHERE l.simulation_id=UUID_TO_BIN(?)
      AND l.entity_id<>UUID_TO_BIN(?)
    ORDER BY RAND() LIMIT 1
  `,[simulationId, originId || entityId]);
  return rows[0]?.locationId || null;
}

async function createAction({simulationId,entityId,decisionId,actionType,simulationTime,targetEntityId=null,targetLocationId=null,sourceType="AUTONOMOUS",sourceGoalId=null,sourceIntentionId=null,sourceEventId=null}) {
  const id=uuid();
  const params={actionType,targetEntityId,targetLocationId};
  await pool.query(`
    INSERT INTO actions
      (id,simulation_id,entity_id,decision_id,action_type,source_type,source_goal_id,source_intention_id,
       source_event_id,started_simulation_at,status,target,parameters,version)
    VALUES(UUID_TO_BIN(?),UUID_TO_BIN(?),UUID_TO_BIN(?),UUID_TO_BIN(?),?,?,UUID_TO_BIN(?),UUID_TO_BIN(?),
           UUID_TO_BIN(?),?,'ACTIVE',UUID_TO_BIN(?),?,1)
  `,[id,simulationId,entityId,decisionId,actionType,sourceType,sourceGoalId,sourceIntentionId,
     sourceEventId,simulationTime,targetLocationId,JSON.stringify(params)]);
  return id;
}

async function executeAction({simulationId,entityId,decisionId,intentionId=null,actionType,simulationTime,targetEntityId=null,targetLocationId=null}) {
  let actionId;
...
`, [
  ...
  if(actionType==="WALKING" || actionType==="EXPLORING"){
    ...
    if(origin && destination && origin!==destination){
      await moveEntity(simulationId,entityId,origin,destination,simulationTime,actionId);
    }
  }
  ...
  `,[simulationTime,JSON.stringify({eventId,actionType}),actionId]);
  if(intentionId){
    await pool.query(`
  ...
  `,[JSON.stringify({actionId,eventId,success:true}),decisionId]);
  return {actionId,eventId};
}

async function moveEntity(simulationId,entityId,origin,destination,simulationTime,sourceActionId){
  const [existing]=await pool.query(`
    SELECT BIN_TO_UUID(id) AS id,version FROM movements
  ...
  await pool.query(`
    ...
    WHERE simulation_id=UUID_TO_BIN(?) AND entity_id=UUID_TO_BIN(?)
      AND location_id=UUID_TO_BIN(?) AND exited_simulation_at IS NULL
  ...
  await pool.query(`
    ...
    ON DUPLICATE KEY UPDATE location_id=VALUES(location_id),since_simulation_at=VALUES(since_simulation_at),
      reason=VALUES(reason),version=version+1
  ...
  await pool.query(`
    ...
    WHERE simulation_id=UUID_TO_BIN(?) AND entity_id=UUID_TO_BIN(?) AND status='ACTIVE'
...
module.exports={createAction,executeAction,learnFromAction};
