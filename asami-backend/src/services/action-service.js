...
function nextHop(locations,originId,targetId=null,avoidLocationId=null){if(!originId)return null;if(targetId)return shortestRoute(locations,originId,targetId)?.path[1]||null;const byId=new Map(locations.map(location=>[String(location.locationId),location])),byCode=new Map(locations.filter(location=>location.data?.worldCode).map(location=>[String(location.data.worldCode).toUpperCase(),location])),origin=byId.get(String(originId));if(!origin)return null;let fallback=null;for(const connection of Array.isArray(origin.data?.connections)?origin.data.connections:[]){const next=resolveConnection(byId,byCode,connection);if(!next||String(next.locationId)===String(originId))continue;if(!fallback)fallback=next.locationId;if(avoidLocationId&&String(next.locationId)===String(avoidLocationId))continue;return next.locationId;}return fallback;}
...
async function startAction({simulationId,entityId,decisionId,intentionId=null,actionType,simulationTime,targetEntityId=null,targetLocationId=null,movementAvoidLocationId=null,relationshipIntent="NONE",tickId=null}){
  if(decisionId){
  ...
  try{
    if(MOVE_ACTIONS.has(normalizedAction)){
      ...
      if(targetLocationId){
        ...
        const locations=await loadLocationGraph(simulationId),next=nextHop(locations,origin,null,movementAvoidLocationId);
    ...
    if(decisionId){
      try{
        assertTransition("decision","EVALUATED","FAILED");
        await pool.query(
