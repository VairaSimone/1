...
const NPC_PROFILES = [
  ...
  { firstName:"Davide", lastName:"Ferrari", description:"Energetic, playful and competitive.", preferred:"GYM", traits:{EXTRAVERSION:.72,SOCIABILITY:.70,OPENNESS:.58,IMPULSIVITY:.72,CONFIDENCE:.76,SPORTS:.70} },
...
async function ensurePartnerType(){
  await pool.query(`
    ...
    VALUES(UUID_TO_BIN(?),'PARTNER','Partner',1,?,1)
    ON DUPLICATE KEY UPDATE name=VALUES(name),symmetric=VALUES(symmetric),configuration=VALUES(configuration),active=1
  `,[PARTNER_RELATIONSHIP_TYPE_ID,JSON.stringify({romantic:true})]);
}

async function getLocationRows(simulationId){
  const [rows]=await pool.query(`
    SELECT BIN_TO_UUID(e.id) AS locationId,e.display_name AS name,e.attributes,l.location_type AS locationType,l.latitude,l.longitude,l.address_data AS addressData
    FROM entities e JOIN locations l ON l.entity_id=e.id
...
async function seedLocations(simulationId,simulationTime){
  for(const location of WORLD_LOCATIONS){
    ...
    `,[entityId,simulationId,LOCATION_ENTITY_TYPE_ID,location.name,location.description,JSON.stringify({worldCode:location.code,connections:location.connections}),simulationTime]);
    await pool.query(`
      INSERT INTO locations(entity_id,location_type,latitude,longitude,address_data)
      VALUES(UUID_TO_BIN(?),?,?,?,?)
    `,[entityId,location.type,location.lat,location.lon,JSON.stringify({worldCode:location.code,description:location.description,connections:location.connections})]);
  }
...
async function createPerson(simulationId,simulationTime,profile,randomSpawn=false){
  ...
  const attributes={
    worldResident:true,
    npc:true,
    role:randomSpawn?"NEIGHBOR":"NEIGHBOR",
...
async function ensureWorld(simulationId,simulationTime){
  ...
  const [unplaced]=await pool.query(`
    ...
    FROM entities e
    WHERE e.simulation_id=UUID_TO_BIN(?) AND e.entity_type_id=UUID_TO_BIN(?) AND e.status='ACTIVE'
      AND NOT EXISTS(SELECT 1 FROM entity_locations_current elc WHERE elc.simulation_id=e.simulation_id AND elc.entity_id=e.id)
    LIMIT 50
  `,[simulationId,PERSON_ENTITY_TYPE_ID]);
...
async function evolveRelationships(simulationId,simulationTime){
  await ensurePartnerType();
  const [rows]=await pool.query(`
    SELECT BIN_TO_UUID(r.id) AS id,BIN_TO_UUID(r.source_entity_id) AS sourceEntityId,BIN_TO_UUID(r.target_entity_id) AS targetEntityId,
           rt.code AS type,r.affection_score affection,r.trust_score trust,r.familiarity_score familiarity,r.closeness_score closeness,
           r.conflict_score conflict,r.version
  ...
  for(const rel of rows){
    ...
    if(rel.type==='PARTNER'){
      const [lastTalk]=await pool.query(`
        SELECT MAX(started_simulation_at) AS lastTalk FROM actions
        WHERE simulation_id=UUID_TO_BIN(?) AND action_type='TALKING' AND status='COMPLETED'
          AND ((entity_id=UUID_TO_BIN(?) AND JSON_UNQUOTE(JSON_EXTRACT(parameters,'$.targetEntityId'))=?)
            OR (entity_id=UUID_TO_BIN(?) AND JSON_UNQUOTE(JSON_EXTRACT(parameters,'$.targetEntityId'))=?))
      `,[simulationId,rel.sourceEntityId,rel.targetEntityId,rel.targetEntityId,rel.sourceEntityId]);
      ...
      if((Number(rel.conflict)>0.72 && Math.random()<0.15) || (hoursAway>72 && Math.random()<0.08)){
        await endRelationship(rel.id,simulationTime,hoursAway>72?"NEGLECT":"CONFLICT");
      }
    }
  }
  ...
  for(const person of people){
    const [partnerRows]=await pool.query(`
      SELECT r.id FROM relationships r JOIN relationship_types rt ON rt.id=r.relationship_type_id
      WHERE r.simulation_id=UUID_TO_BIN(?) AND rt.code='PARTNER' AND r.status='ACTIVE' AND (r.source_entity_id=UUID_TO_BIN(?) OR r.target_entity_id=UUID_TO_BIN(?)) LIMIT 1
    `,[simulationId,person.id,person.id]);
    if(partnerRows.length)continue;
    const [candidates]=await pool.query(`
      SELECT BIN_TO_UUID(r.id) AS id,BIN_TO_UUID(CASE WHEN r.source_entity_id=UUID_TO_BIN(?) THEN r.target_entity_id ELSE r.source_entity_id END) AS candidateId,
             r.familiarity,r.closeness,r.affection,r.trust
      FROM relationships r JOIN relationship_types rt ON rt.id=r.relationship_type_id
      WHERE r.simulation_id=UUID_TO_BIN(?) AND r.status='ACTIVE' AND rt.code IN ('FRIEND','ACQUAINTANCE')
        AND (r.source_entity_id=UUID_TO_BIN(?) OR r.target_entity_id=UUID_TO_BIN(?))
      ORDER BY (r.closeness+r.affection+r.familiarity+r.trust) DESC LIMIT 5
    `,[person.id,simulationId,person.id,person.id]);
    for(const candidate of candidates){
      ...
      const [otherPartner]=await pool.query(`
        SELECT r.id FROM relationships r JOIN relationship_types rt ON rt.id=r.relationship_type_id
        WHERE r.simulation_id=UUID_TO_BIN(?) AND rt.code='PARTNER' AND r.status='ACTIVE' AND (r.source_entity_id=UUID_TO_BIN(?) OR r.target_entity_id=UUID_TO_BIN(?)) LIMIT 1
      `,[simulationId,candidate.candidateId,candidate.candidateId]);
      ...
      for(const rel of ['FRIEND','ACQUAINTANCE']){
...
module.exports={ensureWorld,evolveRelationships,seedLocations,ensurePopulation,MIN_WORLD_PEOPLE,MAX_WORLD_PEOPLE};
