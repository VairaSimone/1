...
async function locationRow(simulationId,locationId){
  const [rows]=await pool.query(`SELECT BIN_TO_UUID(e.id) AS locationId,e.attributes,e.version,l.location_type AS locationType FROM entities e JOIN locations l ON l.entity_id=e.id AND l.simulation_id=e.simulation_id WHERE e.simulation_id=UUID_TO_BIN(?) AND e.id=UUID_TO_BIN(?) AND e.entity_type_id=UUID_TO_BIN(?) AND e.status='ACTIVE' LIMIT 1`,[simulationId,locationId,LOCATION_ENTITY_TYPE_ID]);
  return rows[0]||null;
}
async function updateLocationAttributes(simulationId,locationId,updater){
  for(let attempt=0;attempt<3;attempt++){
    const row=await locationRow(simulationId,locationId);if(!row)return null;
    const next=updater(parseJson(row.attributes,{}));
    const [updated]=await pool.query(`UPDATE entities SET attributes=?,version=version+1 WHERE id=UUID_TO_BIN(?) AND simulation_id=UUID_TO_BIN(?) AND version=?`,[JSON.stringify(next),locationId,simulationId,Number(row.version||1)]);
    if(updated.affectedRows)return next;
  }
  return null;
}
async function seedPhysicalWorld(simulationId,simulationTime){
  const [rows]=await pool.query(`SELECT BIN_TO_UUID(e.id) AS locationId,e.attributes,e.version,l.location_type AS locationType FROM entities e JOIN locations l ON l.entity_id=e.id AND l.simulation_id=e.simulation_id WHERE e.simulation_id=UUID_TO_BIN(?) AND e.entity_type_id=UUID_TO_BIN(?) AND e.status='ACTIVE'`,[simulationId,LOCATION_ENTITY_TYPE_ID]);
  for(const row of rows){
    const attributes=parseJson(row.attributes,{}),code=attributes.worldCode||row.locationType;if(!LOCATION_RESOURCES[code])continue;
    ...
    await pool.query(`UPDATE entities SET attributes=?,version=version+1 WHERE id=UUID_TO_BIN(?) AND simulation_id=UUID_TO_BIN(?) AND version=?`,[JSON.stringify(next),row.locationId,simulationId,Number(row.version||1)]);
  }
}
async function getLocationPhysicalState(simulationId,locationId){
  const row=await locationRow(simulationId,locationId);if(!row)return null;const a=parseJson(row.attributes,{});
...
async function replenishResource({simulationId,locationId,resource,amount,simulationTime}){
  ...
  const next=await updateLocationAttributes(simulationId,locationId,current=>{const resources={...(current.resources||{})};remaining=clamp(resources[resource],0)+quantity;resources[resource]=remaining;return {...current,resources,physicalUpdatedAt:simulationTime};});
