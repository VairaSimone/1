...
function buildDeliberativeLayer({ needs = [], goals = [], activePlanStep = null, memories = [], social = null } = {}) {
  const deliberativeNeeds = (needs || []).filter(need => DELIBERATIVE_NEEDS.has(normalize(need.code))).map(need => ({ code: normalize(need.code), value: clamp01(need.value), priorityWeight: Number(need.priorityWeight || 1) })).sort((a, b) => (b.value * b.priorityWeight) - (a.value * a.priorityWeight));
  const activeGoal = (goals || []).filter(goal => Number(goal.progress || 0) < 1).sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0))[0] || null;
  return { kind: "DELIBERATIVE", goal: activeGoal ? { id: activeGoal.id, title: activeGoal.title, priority: Number(activeGoal.priority || 0), progress: Number(activeGoal.progress || 0), motivation: Number(activeGoal.motivation || 0) } : null, activePlanStep: activePlanStep ? { id: activePlanStep.id, sequence: Number(activePlanStep.sequence || 0), title: activePlanStep.title, actionType: normalize(activePlanStep.result?.actionType || activePlanStep.actionType), status: normalize(activePlanStep.status) } : null, dominantNeeds: deliberativeNeeds.slice(0, 4), socialOpportunityCount: Array.isArray(social?.candidates) ? social.candidates.length : 0, memoryCount: Array.isArray(memories) ? memories.length : 0, objective: activeGoal ? "ADVANCE_GOAL" : deliberativeNeeds.length ? "REDUCE_DELIBERATIVE_PRESSURE" : "MAINTAIN_STATE" };
}

function getHabitAction(habit) { return normalize(habit?.actionDefinition?.actionType || habit?.actionType); }
...
function buildHabitLayer({ habits = [], needs = [], simulationTime } = {}) {
  ...
  const candidates = (habits || []).map(habit => { const triggerMatch = habitTriggerMatch(habit, simulationTime); const strength = clamp01(habit.strength); return { id: habit.id, name: habit.name, actionType: getHabitAction(habit), strength, triggerMatch, score: strength * triggerMatch }; }).filter(candidate => candidate.actionType && candidate.score > 0).sort((a, b) => b.score - a.score);
...
function composeCognitiveLayers({ needs = [], goals = [], activePlanStep = null, memories = [], social = null, habits = [], simulationTime } = {}) {
  ...
  return { reactive, deliberative, habit, precedence: ["REACTIVE", "DELIBERATIVE", "HABIT"], winner: reactive.blocking ? "REACTIVE" : habit.selected && !deliberative.goal ? "HABIT" : "DELIBERATIVE" };
}
