...
const RESOURCE_SEARCH_TTL_MINUTES=180,RESOURCE_TRAVEL_BONUS=.85,WALKING_SPEED_KMH=4.8,ROAD_FACTOR=1.18,MAX_EXPERIENCE_SCORE_EFFECT=.65,MAX_RECENT_ACTIONS=12,MAX_RECENT_INTERRUPTION_HOURS=12,STOCHASTIC_TOP_K=4,BASE_TEMPERATURE=.24;
...
function applyRecoveryBlocks(candidates,recoveryBlocks=[]){
...
function chooseSocialTargetCandidate(context,entityId){const candidates=Array.isArray(context?.social?.candidates)?context.social.candidates:[];if(!candidates.length)return null;return candidates.map(candidate=>{const familiarity=Number(candidate.familiarity||0),closeness=Number(candidate.closeness||0),affection=Number(candidate.affection||0),trust=Number(candidate.trust||0),compatibility=Number(candidate.compatibility||.5),romantic=Number(candidate.romanticScore||0),relationshipValue=familiarity*.70+closeness*.85+affection*.95+trust*.80+compatibility*.75+romantic*.18,novelty=(1-Math.min(1,familiarity))*.28,pairBias=socialPairBias(entityId,candidate.id),returnScore=relationshipValue+novelty+pairBias;return{...candidate,selectionScore:returnScore};}).sort((a,b)=>Number(b.selectionScore||0)-Number(a.selectionScore||0))[0]||null;}
