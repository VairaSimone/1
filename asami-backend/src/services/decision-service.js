...
async function buildDecisionContexts(simulationId,entityIds=[],simulationTime=null){
  const ids=[...new Set((entityIds||[]).filter(Boolean).map(String))],contexts=new Map();
  if(!ids.length)return contexts;
  let effectiveSimulationTime=simulationTime;
  if(!effectiveSimulationTime){const[rows]=await pool.query(`SELECT current_simulation_at AS currentSimulationAt FROM simulations WHERE id=UUID_TO_BIN(?) LIMIT 1`,[simulationId]);effectiveSimulationTime=rows[0]?.currentSimulationAt||new Date();}
  const placeholders=ids.map(()=> 'UUID_TO_BIN(?)').join(',');
  const [needs,traits,goals,locations,recentActions,recentInterruptions,recentSocialInteractions,worldRows,cognitiveProfiles]=await Promise.all([
    pool.query(`SELECT BIN_TO_UUID(enc.entity_id) AS entityId,nd.code,enc.value,nd.priority_weight AS priorityWeight FROM entity_needs_current enc JOIN need_definitions nd ON nd.id=enc.need_id WHERE enc.entity_id IN (${placeholders}) AND nd.active=1`,ids),
    ...
    pool.query(`SELECT BIN_TO_UUID(e.id) AS locationId,l.location_type AS locationType,l.latitude,l.longitude,l.address_data AS addressData,e.attributes FROM locations l JOIN entities e ON e.id=l.entity_id WHERE l.simulation_id=UUID_TO_BIN(?) AND e.simulation_id=UUID_TO_BIN(?) AND e.status='ACTIVE'`,[simulationId,simulationId]),
    getCognitiveProfiles(simulationId,ids)
  ]);
  const needsByEntity=new Map(ids.map(id=>[id,[]])),traitsByEntity=new Map(ids.map(id=>[id,[]])),goalsByEntity=new Map(ids.map(id=>[id,[]])),actionsByEntity=new Map(ids.map(id=>[id,[]])),interruptionsByEntity=new Map(ids.map(id=>[id,[]])),socialByEntity=new Map(ids.map(id=>[id,[]])),locationsByEntity=new Map();
  for(const row of needs[0])needsByEntity.get(row.entityId)?.push({code:row.code,value:Number(row.value),priorityWeight:Number(row.priorityWeight)});
  for(const row of traits[0])traitsByEntity.get(row.entityId)?.push({code:row.code,value:Number(row.value)});
  for(const row of goals[0])goalsByEntity.get(row.entityId)?.push({...row,priority:Number(row.priority),progress:Number(row.progress),motivation:parseJson(row.motivation,row.motivation)});
  for(const row of recentActions[0])actionsByEntity.get(row.entityId)?.push(row);
  for(const row of recentInterruptions[0])interruptionsByEntity.get(row.entityId)?.push({...row,result:parseJson(row.result,{})});
  for(const row of recentSocialInteractions[0])socialByEntity.get(row.entityId)?.push(row.targetEntityId);
  for(const row of locations[0])locationsByEntity.set(row.entityId,row);
  const worldLocations=worldRows[0].map(row=>{const attributes=parseJson(row.attributes,{});return{locationId:row.locationId,locationType:row.locationType,latitude:Number(row.latitude),longitude:Number(row.longitude),data:parseJson(row.addressData,{}),resources:attributes.resources&&typeof attributes.resources==='object'?attributes.resources:{},resourceEmergencies:attributes.resourceEmergencies&&typeof attributes.resourceEmergencies==='object'?attributes.resourceEmergencies:{}};});
  for(const id of ids){
    const currentLocation=locationsByEntity.get(id)||null,needsList=needsByEntity.get(id)||[],profile=cognitiveProfiles.get(id)||{mentalState:{currentFocus:null,currentConcern:null,recentThought:null,mentalLoad:.2,rumination:.1,certainty:.5,updatedSimulationAt:null},preferences:[],beliefs:[],knowledge:[],habits:[],plans:[]};
    ...
    const resourceExclusions=criticalResourceNeedState(needsList)?[]:excludedLocationIds,resourceContext={currentLocationId:currentLocation?.locationId||null,currentResources:currentLocation?.resources||{},localResources:currentLocation?.resources||{},blockedResources:{},nearestResources:{},emergencyResources:[],resourceEmergency:null,actions:{}};
    ...
    const currentWorldLocation=worldLocations.find(item=>item.locationId===currentLocation?.locationId);
    ...
    resourceContext.emergencyResources=resourceEmergencyCandidates;for(const resource of ['water','food']){const required=resource==='water'?1:1,localAvailable=Number(currentWorldLocation?.resources?.[resource]??0),emergency=resourceEmergencyCandidates.find(item=>item.resource===resource);resourceContext.actions[resource==='water'?'DRINKING':'EATING']={resource,required,localAvailable,locallyAvailable:localAvailable>=required,recentlyBlocked:false,nearestLocation:resourceContext.nearestResources[resource]||null,emergency:emergency||null};}
...
function resolvePlanCommitment(context){
  const step=context?.activePlanStep;
  if(!step)return null;
