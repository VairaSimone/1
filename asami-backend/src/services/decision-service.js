...
async function buildDecisionContexts(simulationId,entityIds=[],simulationTime=null,{worldLocations:providedWorldLocations=null}={}){
  ...
  const [needs,traits,goals,locations,recentActions,recentInterruptions,recentSocialInteractions,resourceKnowledge,worldRows,cognitiveProfiles]=await Promise.all([
    ...
    providedWorldLocations ? Promise.resolve([null]) : pool.query(`SELECT BIN_TO_UUID(e.id) AS locationId,l.location_type AS locationType,l.latitude,l.longitude,l.address_data AS addressData,e.attributes FROM locations l JOIN entities e ON e.id=l.entity_id WHERE l.simulation_id=UUID_TO_BIN(?) AND e.simulation_id=UUID_TO_BIN(?) AND e.status='ACTIVE'`,[simulationId,simulationId]),
    getCognitiveProfiles(simulationId,ids)
  ]);
  const needsByEntity=new Map(ids.map(id=>[id,[]])),traitsByEntity=new Map(ids.map(id=>[id,[]])),goalsByEntity=new Map(ids.map(id=>[id,[]])),actionsByEntity=new Map(ids.map(id=>[id,[]])),interruptionsByEntity=new Map(ids.map(id=>[id,[]])),socialByEntity=new Map(ids.map(id=>[id,[]])),resourceKnowledgeByEntity=new Map(ids.map(id=>[id,[]])),locationsByEntity=new Map();
  ...
  for(const row of resourceKnowledge[0])resourceKnowledgeByEntity.get(row.entityId)?.push({...row,content:parseJson(row.content,null)});
