...
async function buildDecisionContexts(simulationId,entityIds=[],simulationTime=null){
  ...
  for(const id of ids){
    ...
    const resourceExclusions=criticalResourceNeedState(needsList)?[]:excludedLocationIds,currentWorldLocation=worldLocations.find(item=>item.locationId===currentLocation?.locationId),resourceContext={currentLocationId:currentLocation?.locationId||null,currentResources:currentWorldLocation?.resources||{},localResources:currentWorldLocation?.resources||{},blockedResources:{},nearestResources:{},emergencyResources:[],resourceEmergency:null,actions:{}};
