...
async function analyzeSimulation(simulationId, { from, to } = {}) {
  ...
  const [
    ...
    pool.query(`SELECT bucket at,
      ...
      ) x GROUP BY bucket ORDER BY bucket`, [simulationId, ...eventRange.params, simulationId, ...actionRange.params, simulationId, ...tickRange.params]),
    pool.query(`SELECT
  ...
  const [highlightRows] = await pool.query(`
    ...
    ) x ORDER BY at DESC LIMIT 12`, [simulationId, ...eventRange.params, simulationId, ...actionRange.params]);
