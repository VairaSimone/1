...
async function resolveBehaviorEntityId(simulationId, entityId) {
  if (entityId) return entityId;
  const [rows] = await pool.query(`SELECT BIN_TO_UUID(e.id) AS id FROM entities e JOIN entity_types et ON et.id=e.entity_type_id WHERE e.simulation_id=UUID_TO_BIN(?) AND et.code='PERSON' ORDER BY CASE WHEN LOWER(e.display_name)='asami' THEN 0 ELSE 1 END,e.created_simulation_at ASC LIMIT 1`, [simulationId]);
  return rows[0]?.id || null;
}

async function enrichActiveAction(action, simulationId, entityId) {
  if (!action) return action;
  const [rows] = await pool.query(`SELECT BIN_TO_UUID(a.source_goal_id) AS goalId,BIN_TO_UUID(i.plan_id) AS planId FROM actions a LEFT JOIN intentions i ON i.id=a.source_intention_id WHERE a.id=UUID_TO_BIN(?) AND a.simulation_id=UUID_TO_BIN(?) AND a.entity_id=UUID_TO_BIN(?) LIMIT 1`, [action.id, simulationId, entityId]);
  const link = rows[0] || {};
  ...
  if (link.planId) {
    const [steps] = await pool.query(`SELECT BIN_TO_UUID(id) AS planStepId FROM plan_steps WHERE plan_id=UUID_TO_BIN(?) AND status='ACTIVE' ORDER BY sequence ASC LIMIT 1`, [link.planId]);
...
async function loadUnavailableLocations(simulationId, entityId, simulationTime) {
  const [rows] = await pool.query(`SELECT BIN_TO_UUID(ki.object_entity_id) AS locationId,ki.content,ek.learned_simulation_at AS learnedAt FROM entity_knowledge ek JOIN knowledge_items ki ON ki.id=ek.knowledge_item_id WHERE ek.simulation_id=UUID_TO_BIN(?) AND ek.entity_id=UUID_TO_BIN(?) AND ek.status='ACTIVE' AND ki.knowledge_type='WORLD_EXPERIENCE' AND ki.predicate='RESOURCE_UNAVAILABLE' ORDER BY ek.learned_simulation_at DESC LIMIT 100`, [simulationId, entityId]);
  const cutoff = nowMs(simulationTime) - RESOURCE_TTL_MS, unavailable = { water: new Set(), food: new Set() };
  for (const row of rows) { if (nowMs(row.learnedAt) < cutoff) continue; const payload = parseJson(row.content, null), resource = normalize(payload?.resource).toLowerCase(); if (resource === "water" || resource === "food") unavailable[resource].add(row.locationId); }
  return unavailable;
}

async function loadResourceWorld(simulationId) {
  const [rows] = await pool.query(`SELECT BIN_TO_UUID(e.id) AS locationId,l.location_type AS locationType,l.latitude,l.longitude,l.address_data AS addressData,e.attributes FROM locations l JOIN entities e ON e.id=l.entity_id AND e.simulation_id=l.simulation_id WHERE l.simulation_id=UUID_TO_BIN(?) AND e.status='ACTIVE'`, [simulationId]);
...
async function strengthenResourceReasoning(context, simulationId, entityId, simulationTime) {
  ...
  const unavailable = await loadUnavailableLocations(simulationId, entityId, simulationTime), locations = await loadResourceWorld(simulationId);
  const nearestResources = { water: nearestResource(locations, currentLocationId, "water", unavailable.water), food: nearestResource(locations, currentLocationId, "food", unavailable.food) };
  const localResources = context.resourceContext.localResources || {}, actions = { ...(context.resourceContext.actions || {}) };
  for (const [action, requirement] of Object.entries(RESOURCE_REQUIREMENTS)) {
    ...
    actions[action] = { ...(actions[action] || {}), resource: requirement.resource, required: requirement.amount, localAvailable, locallyAvailable: localAvailable >= requirement.amount, knownUnavailableHere: unavailable[requirement.resource].has(currentLocationId), nearestLocation: nearestResources[requirement.resource] ? { locationId: nearestResources[requirement.resource].locationId, locationType: nearestResources[requirement.resource].locationType, distanceMeters: nearestResources[requirement.resource].distanceMeters, travelMinutes: nearestResources[requirement.resource].travelMinutes } : null };
  }
  const candidates = (context.candidates || []).map((candidate) => ({ ...candidate }));
  for (const [action, requirement] of Object.entries(RESOURCE_REQUIREMENTS)) {
    const candidate = candidates.find((item) => normalize(item.action) === action), walking = candidates.find((item) => normalize(item.action) === "WALKING"), localAvailable = Number(localResources[requirement.resource] || 0) >= requirement.amount, target = nearestResources[requirement.resource];
    if (candidate && !localAvailable) candidate.score = 0;
    if (walking && !localAvailable && target) {
      const pressure = needValue(context.needs, requirement.need), urgency = Math.max(1.8, Math.min(3.8, 2.0 + pressure * 1.6)), currentScore = Number(walking.score || 0), travelPenalty = Math.min(0.8, target.travelMinutes / 60 * 0.5);
  ...
  context.resourceContext = { ...context.resourceContext, localResources, nearestResources, actions, knownUnavailableLocationIds: { water: [...unavailable.water], food: [...unavailable.food] } };
...
async function loadRecentOutcomes(simulationId, entityId, simulationTime, limit = 24) {
  const [rows] = await pool.query(`SELECT action_type AS actionType,JSON_UNQUOTE(JSON_EXTRACT(result,'$.outcome')) AS outcome,JSON_UNQUOTE(JSON_EXTRACT(result,'$.targetLocationId')) AS targetLocationId,JSON_UNQUOTE(JSON_EXTRACT(result,'$.targetEntityId')) AS targetEntityId,completed_simulation_at AS at FROM actions WHERE simulation_id=UUID_TO_BIN(?) AND entity_id=UUID_TO_BIN(?) AND status='COMPLETED' AND completed_simulation_at IS NOT NULL AND completed_simulation_at<=? ORDER BY completed_simulation_at DESC LIMIT ?`, [simulationId, entityId, simulationTime, limit]);
...
function createFailureLoopPattern(failures, adaptationRate) {
  ...
  return { id: "decision-consequence-adaptation", severity, title: "Ciclo decisione → conseguenza → adattamento", detail: `${failures} esiti problematici rilevati; ${Math.round(adaptationRate * 100)}% è stato seguito da un cambio di strategia entro 6 ore simulate.`, count: failures, evidence: { problematicDecisions: failures, adaptationRate } };
}

function computeAdaptation(rows) {
  const ordered = rows.slice().sort((a, b) => nowMs(a.at) - nowMs(b.at)), failures = ordered.filter((row) => row.outcome === "FAILURE" || row.outcome === "PARTIAL");
  if (!failures.length) return { failures: 0, adapted: 0, rate: 0 };
  let adapted = 0;
  for (const failure of failures) { const failureAt = nowMs(failure.at), next = ordered.find((row) => nowMs(row.at) > failureAt && nowMs(row.at) - failureAt <= 6 * 3600000); if (!next) continue; if (normalize(next.actionType) !== normalize(failure.actionType) || String(next.targetLocationId || "") !== String(failure.targetLocationId || "")) adapted += 1; }
...
async function patchAnalysisResult(result, simulationId, entityId, actualFrom, actualTo) {
  const effectiveEntityId = await resolveBehaviorEntityId(simulationId, entityId);
  const actionParams = [simulationId, actualFrom, actualTo], entityClause = effectiveEntityId ? " AND a.entity_id=UUID_TO_BIN(?)" : "";
  if (effectiveEntityId) actionParams.push(effectiveEntityId);
  const [actions] = await pool.query(`SELECT action_type AS actionType,JSON_UNQUOTE(JSON_EXTRACT(result,'$.outcome')) AS outcome,JSON_UNQUOTE(JSON_EXTRACT(result,'$.targetLocationId')) AS targetLocationId,JSON_UNQUOTE(JSON_EXTRACT(result,'$.targetEntityId')) AS targetEntityId,completed_simulation_at AS at FROM actions a WHERE a.simulation_id=UUID_TO_BIN(?) AND a.started_simulation_at>=? AND a.started_simulation_at<=?${entityClause} ORDER BY completed_simulation_at ASC,started_simulation_at ASC`, actionParams);
  const normalizedActions = actions.map((row) => ({ ...row, outcome: normalize(row.outcome) })), successful = normalizedActions.filter((row) => row.outcome === "SUCCESS").length, problematic = normalizedActions.filter((row) => row.outcome === "FAILURE" || row.outcome === "PARTIAL").length, unsettled = normalizedActions.filter((row) => !row.outcome).length, technicalCompleted = normalizedActions.length - unsettled, settled = successful + problematic, adaptation = computeAdaptation(normalizedActions);

  const decisionParams = [simulationId, actualFrom, actualTo], decisionEntityClause = effectiveEntityId ? " AND d.entity_id=UUID_TO_BIN(?)" : "";
  if (effectiveEntityId) decisionParams.push(effectiveEntityId);
  const [decisionRows] = await pool.query(`SELECT d.id,d.status,JSON_UNQUOTE(JSON_EXTRACT(d.actual_outcome,'$.outcome')) AS outcome,d.simulation_time AS at FROM decisions d WHERE d.simulation_id=UUID_TO_BIN(?) AND d.simulation_time>=? AND d.simulation_time<=?${decisionEntityClause} ORDER BY d.simulation_time ASC`, decisionParams);
  const executedDecisions = decisionRows.filter((row) => row.status === "EXECUTED").length, successfulDecisions = decisionRows.filter((row) => normalize(row.outcome) === "SUCCESS").length, problematicDecisions = decisionRows.filter((row) => normalize(row.outcome) === "FAILURE" || normalize(row.outcome) === "PARTIAL").length, decisionsWithOutcome = successfulDecisions + problematicDecisions;

  const memoryParams = [simulationId, actualFrom, actualTo], memoryEntityClause = effectiveEntityId ? " AND m.entity_id=UUID_TO_BIN(?)" : "";
  if (effectiveEntityId) memoryParams.push(effectiveEntityId);
  const [learningRows] = await pool.query(`SELECT COUNT(*) AS total FROM memories m WHERE m.simulation_id=UUID_TO_BIN(?) AND m.created_simulation_at>=? AND m.created_simulation_at<=?${memoryEntityClause} AND LOWER(COALESCE(JSON_UNQUOTE(JSON_EXTRACT(m.metadata,'$.kind')),'')) IN ('resource_failure','action_failure','action_interruption')`, memoryParams);
  ...
  const loopPattern = createFailureLoopPattern(problematicDecisions || problematic, adaptation.rate); if (loopPattern) result.patterns.unshift(loopPattern);
  return result;
}
...
function install() {
  ...
  actionService.getActiveAction = async function patchedGetActiveAction(entityId, simulationId) { const action = await originalGetActiveAction(entityId, simulationId); return enrichActiveAction(action, simulationId, entityId); };
  const originalCompleteAction = actionService.completeAction;
  actionService.completeAction = async function patchedCompleteAction(args = {}) { const result = await originalCompleteAction(args); if (result?.completed && result?.outcome && args?.actionId && !feedbackApplied.has(args.actionId)) { feedbackApplied.add(args.actionId); try { await applyActionOutcomeNeedFeedback({ entityId: args.entityId, actionId: args.actionId, actionType: args.actionType, simulationTime: args.simulationTime, outcome: result.outcome }); } catch { feedbackApplied.delete(args.actionId); } } return result; };
  const originalBuildDecisionContext = decisionService.buildDecisionContext;
  decisionService.buildDecisionContext = async function patchedBuildDecisionContext(simulationId, entityId, simulationTime) { let context = await originalBuildDecisionContext(simulationId, entityId, simulationTime); context.recentOutcomes = await loadRecentOutcomes(simulationId, entityId, simulationTime); context = await strengthenResourceReasoning(context, simulationId, entityId, simulationTime); context.candidates = applyFailurePenalty(context.candidates || [], context.recentOutcomes); return context; };
  ...
  decisionService.makeDecision = async function patchedMakeDecision(args = {}) {
    ...
    let context = { ...sourceContext, candidates: (sourceContext.candidates || []).map((candidate) => ({ ...candidate })) };
    const localResources = context.resourceContext?.localResources || {}, thirstBlocked = needValue(context.needs, "THIRST") >= 0.8 && Number(localResources.water || 0) < 1, hungerBlocked = needValue(context.needs, "HUNGER") >= 0.8 && Number(localResources.food || 0) < 1, blockedResources = [];
    if (thirstBlocked) blockedResources.push("THIRST"); if (hungerBlocked) blockedResources.push("HUNGER");
    if (blockedResources.length) {
      context.needs = (context.needs || []).map((need) => blockedResources.includes(normalize(need.code)) ? { ...need, value: 0.799 } : need);
      if (context.activePlanStep) { const stepAction = normalize(context.activePlanStep.actionType || context.activePlanStep.result?.actionType); if ((stepAction === "DRINKING" && thirstBlocked) || (stepAction === "EATING" && hungerBlocked)) context.activePlanStep = null; }
  ...
  analysisService.analyzeSimulation = async function patchedAnalyzeSimulation(simulationId, options = {}) { const result = await originalAnalyzeSimulation(simulationId, options); return patchAnalysisResult(result, simulationId, options.entityId, result.range.from, result.range.to); };
  installed = true;
}
