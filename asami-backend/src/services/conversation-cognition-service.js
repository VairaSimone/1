...
async function updateCommunicationStyle(simulationId,entityId,proposed,simulationTime){if(!proposed||typeof proposed!=="object")return null;const defaults={formality:.45,warmth:.6,directness:.55,verbosity:.45,humor:.25,emojiUse:.08,emotionalOpenness:.55,argumentativeDepth:.6};for(let attempt=0;attempt<3;attempt+=1){const[rows]=await pool.query(`SELECT attributes,version FROM entities WHERE simulation_id=UUID_TO_BIN(?) AND id=UUID_TO_BIN(?) LIMIT 1`,[simulationId,entityId]);if(!rows.length)return null;let attributes=rows[0].attributes||{};if(typeof attributes==='string'){try{attributes=JSON.parse(attributes);}catch{attributes={};}}const previous=attributes.communicationStyle&&typeof attributes.communicationStyle==='object'?attributes.communicationStyle:{};const next={updatedSimulationAt:simulationTime};for(const key of Object.keys(defaults)){const base=clamp01(previous[key]??defaults[key]);const requested=proposed[key];const evolution=Number.isFinite(Number(requested))?clampDelta(Number(requested)-base,.05):0;next[key]=clamp01(base+evolution);}attributes.communicationStyle=next;const[updated]=await pool.query(`UPDATE entities SET attributes=?,version=version+1 WHERE simulation_id=UUID_TO_BIN(?) AND id=UUID_TO_BIN(?) AND version=?`,[JSON.stringify(attributes),simulationId,entityId,rows[0].version]);if(updated.affectedRows)return next;}return null;}
