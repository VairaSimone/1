...
const { buildSocialContext, buildSocialContexts, deriveSocialIntent } = require("./social-relationship-service");
...
function chooseSocialTarget(socialContext,simulationId,entityId,originId){const candidates=Array.isArray(socialContext?.candidates)?socialContext.candidates:[];if(candidates.length){const ranked=candidates.slice().sort((a,b)=>{const aPartner=socialContext.partner?.partnerId===a.id?1:0,bPartner=socialContext.partner?.partnerId===b.id?1:0,aScore=Number(a.score||0)+aPartner*.35+Number(a.romanticScore||0)*.35,bScore=Number(b.score||0)+bPartner*.35+Number(b.romanticScore||0)*.35;return bScore-aScore;});const target=ranked[0];return{entityId:target.id,name:target.name,locationId:originId,remote:false,score:Number(target.score||0)};}const remoteCandidates=Array.isArray(socialContext?.remoteCandidates)?socialContext.remoteCandidates:[];if(remoteCandidates.length){const target=remoteCandidates[0];return{entityId:target.id,name:target.name,locationId:target.locationId,remote:true,score:Number(target.score||0),travelMinutes:Number(target.travelMinutes||0),distanceMeters:Number(target.distanceMeters||0)};}return chooseRemoteSocialTarget(simulationId,entityId,originId);}
...
async function actForEntity({simulationId,entityId,simulationTime,gemini,tickId=null,batchContext=null}){const entity=batchContext?.entities?.get(entityId)||await getEntity(simulationId,entityId);if(!entity)return null;let context=batchContext?.contexts?.get(entityId)||await decisionService.buildDecisionContext(simulationId,entityId,simulationTime);context={...context,simulationTime};if(batchContext?.contexts?.has(entityId)){const latestNeeds=await readNeeds(entityId);if(Array.isArray(latestNeeds)&&latestNeeds.length)context={...context,needs:latestNeeds,recoveryBlocks:decisionService.activeRecoveryBlocks(context.recentInterruptions||[],latestNeeds),candidates:decisionService.rebuildDecisionCandidates({...context,recoveryBlocks:decisionService.activeRecoveryBlocks(context.recentInterruptions||[],latestNeeds)},entityId,latestNeeds),needPriority:decisionService.needPriorityState?decisionService.needPriorityState(latestNeeds):context.needPriority};}const goalState=await ensureGoalPlan({simulationId,entityId,simulationTime,needs:context.needs});if(goalState.created||(goalState.goal&&!context.goals.some(goal=>goal.id===goalState.goal.id)))context=await decisionService.buildDecisionContext(simulationId,entityId,simulationTime);context.simulationTime=simulationTime;const socialContext=batchContext?.socialContexts?.get(entityId)||(entity.entityType==="PERSON"?await buildSocialContext(simulationId,entityId):{partner:null,candidates:[]});context.social={partner:socialContext.partner,candidates:socialContext.candidates.map(candidate=>({id:candidate.id,name:candidate.name,relationshipType:candidate.relationshipType,compatibility:Number(candidate.compatibility.toFixed(3)),romanticScore:Number(candidate.romanticScore.toFixed(3)),familiarity:Number(candidate.relationship?.familiarity||0),closeness:Number(candidate.relationship?.closeness||0),affection:Number(candidate.relationship?.affection||0),trust:Number(candidate.relationship?.trust||0)}))};
  ...
  if(!context.social.candidates.length&&!socialContext.remoteCandidates?.length&&socialPressure>=.55){
