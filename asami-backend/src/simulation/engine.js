...
class SimulationEngine {
  ...
  async runSimulation(sim){
    ...
    try{
      const clock=await simRepo.getActiveClock(sim.id);if(!clock)return;const nextTime=new Date(new Date(clock.simulationAnchorAt).getTime()+(Date.now()-new Date(clock.realAnchorAt).getTime())*Number(clock.speed));const previousTime=new Date(sim.currentSimulationAt||clock.simulationAnchorAt);if(nextTime<=previousTime)return;const advanced=await simRepo.updateCurrentTimeOptimistic(sim.id,nextTime,sim.version);if(!advanced)return;context.simulationTime=nextTime.toISOString();
      phase="tick.create";tickId=await simRepo.createTick(sim.id,nextTime,"AUTONOMOUS",env.ENGINE_VERSION);
      try{
        const elapsedMinutes=Math.min(360,Math.max(0,(nextTime-previousTime)/60000));const lastMaintenance=this.worldMaintenanceAt.get(sim.id);const maintenanceDue=lastMaintenance===undefined||nextTime.getTime()-lastMaintenance>=3600000;
        ...
        for(const id of actors){
          ...
          if(active){
            ...
            phase="entity.mental_state";if(["TALKING","STUDYING","WORKING","EXPLORING"].includes(active.actionType))await updateMentalState(sim.id,entityId,nextTime,{currentFocus:active.actionType.toLowerCase().replaceAll("_"," "),mentalLoad:["WORKING","STUDYING"].includes(active.actionType)?0.55:0.35,certainty:0.7});phase="entity.publish";this.hub.publish(sim.id,"entity.state",{entityId,action:{...active,status:wasCompleted?"COMPLETED":"ACTIVE",startedSimulationAt:active.startedSimulationAt,expectedCompletionSimulationAt:completionAt,relationshipIntent},needChanges});continue;
          }
          ...
          await pool.query(`UPDATE actions SET result=? WHERE id=UUID_TO_BIN(?)`,[JSON.stringify({eventId:action.eventId,actionType:action.actionType,durationMinutes:action.durationMinutes,targetEntityId:decision.targetEntityId||null,targetLocationId:decision.targetLocationId||null,goalId:decision.goalId||null,relationshipIntent:decision.relationshipIntent||"NONE",expectedCompletionSimulationAt:action.expectedCompletionSimulationAt,movement:action.movement||null}),action.actionId]);phase="entity.publish";this.hub.publish(sim.id,"entity.state",{entityId,decision,action,status:"ACTIVE",needChanges});
        }
