...
class SimulationEngine {
  constructor({ gemini, hub }) {
    ...
    this.worldMaintenanceAt = new Map();
  }

  async start() {
    if (this.interval) return;
    this.interval = setInterval(
      () => this.pulse().catch(err => logger.error(logger.contextError({ phase: "pulse" }, err, "engine pulse failed"))),
      env.ENGINE_INTERVAL_MS
    );
    await this.pulse();
  }
  ...
  async stop({ drainTimeoutMs = 5000 } = {}) {
    ...
    if (this.running.size) logger.warn({ activeSimulations: this.running.size }, "engine shutdown timeout reached; stopping with active simulations");
  }
  ...
  async runSimulation(sim) {
    ...
    try {
      ...
      const previousTime = new Date(sim.currentSimulationAt || clock.simulationAnchorAt);
      ...
      try {
        ...
        phase = "world.events";
        await generateWorldEvents(sim.id, nextTime, tickId, elapsedMinutes);
        ...
        for (const id of actors) {
          ...
          if (active) {
            ...
            const relationshipIntent = active.metadata?.relationshipIntent || "NONE";
            ...
            phase = "entity.needs";
            const needChanges = await updateNeeds(entityId, updateTime, updateHours, null, active.id, active.actionType);
            phase = "entity.emotions";
            await applyEmotions(entityId, updateTime, needChanges, null, active.id, active.actionType, updateHours);
            ...
            if (wasCompleted) {
              ...
              if (!completion?.completed) {
                logger.warn({ simulationId: sim.id, entityId, actionId: active.id }, "action completion was not committed; skipping downstream learning");
                continue;
              }
              ...
              if (successful) await learnFromAction(entityId, active.actionType, completionAt);
              phase = "entity.development";
              if (successful) await updateDevelopment(sim.id, entityId, completionAt);
              phase = "entity.traits";
              if (successful) await developTraits(entityId, completionAt, signalForDecision(active.actionType), null, active.id);
              phase = "entity.goal";
              if (successful) await completeGoalForAction(active.metadata?.goalId || null, active.actionType, completionAt);
              phase = "entity.habit";
              if (successful) await recordHabitEvidence({ entityId, simulationTime: completionAt, actionType: active.actionType });
              phase = "entity.memory";
              if (outcome !== "FAILURE") {
                await createMemory({
                  simulationId: sim.id,
                  entityId,
                  eventId,
                  content: buildActionMemoryContent(active, completion),
                  importance: outcome === "PARTIAL" ? 0.6 : 0.45,
                  strength: 0.9,
                  confidence: 0.8,
                  emotionalIntensity: outcome === "PARTIAL" ? 0.3 : 0.25,
                  ...
                  metadata: {
                    ...
                    resource: completion.resource || null,
                    resourceLearning: completion.resourceLearning || null,
                    perceptionSummary: perception.location || null,
                    durationMinutes,
                    relationshipIntent
                  }
                });
            ...
            if (["TALKING", "STUDYING", "WORKING", "EXPLORING"].includes(active.actionType)) {
              await updateMentalState(sim.id, entityId, nextTime, {
                currentFocus: active.actionType.toLowerCase().replaceAll("_", " "),
                mentalLoad: ["WORKING", "STUDYING"].includes(active.actionType) ? 0.55 : 0.35,
                certainty: 0.7
              });
            }
            ...
            this.hub.publish(sim.id, "entity.state", {
              ...
              action: {
                ...
                startedSimulationAt: active.startedSimulationAt,
                expectedCompletionSimulationAt: completionAt,
                relationshipIntent
              },
              needChanges
            });
          ...
          const perception = await perceive(sim.id, entityId, nextTime);
          phase = "entity.needs";
          const needChanges = await updateNeeds(entityId, nextTime, elapsedHours);
          phase = "entity.emotions";
          await applyEmotions(sim.id ? entityId : entityId, nextTime, needChanges, null, null, null, elapsedHours);
          ...
          if (!decision) {
            phase = "entity.publish";
            this.hub.publish(sim.id, "entity.state", { entityId, decision: null, action: null, status: "IDLE", needChanges });
            continue;
          }
          ...
          const action = await startAction({
            ...
            targetLocationId: decision.targetLocationId || null,
            relationshipIntent: decision.relationshipIntent || "NONE"
          });
          ...
          await pool.query(
            ...
            [
              JSON.stringify({
                ...
                relationshipIntent: decision.relationshipIntent || "NONE",
                expectedCompletionSimulationAt: action.expectedCompletionSimulationAt,
          ...
          phase = "entity.publish";
          this.hub.publish(sim.id, "entity.state", { entityId, decision, action, status: "ACTIVE", needChanges });
        }
        ...
        if (count % 600 === 0) {
          ...
          if (asami) await initiateConversation({ simulationId: sim.id, asamiEntityId: asami.id, simulationTime: nextTime, gemini: this.gemini, hub: this.hub });
        }
...
function buildActionMemoryContent(action, completion) {
  ...
  if (completion.outcome === "PARTIAL") {
    return `I tried to ${actionLabel}, but only part of the expected result was achieved. I should consider this outcome in future decisions.`;
  }
...
async function buildSnapshot(simulationId, simulationTime) {
  ...
  for (const actor of actors) {
    const dashboard = await entityRepo.getDashboard(simulationId, actor.id);
    entities.push({
