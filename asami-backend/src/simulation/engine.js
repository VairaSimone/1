...
class SimulationEngine {
  ...
  async runSimulation(sim){
    const context={simulationId:sim.id,simulationVersion:sim.version,simulationTime:sim.currentSimulationAt||null}; let tickId=null,phase="clock",entityId=null,actionType=null;
    try{
      ...
      try{
        ...
        for(const id of actors){
          ...
          if(active){
            ...
            const tickActionEnd=nextTime>=completionAt?completionAt:nextTime;
            ...
            phase="entity.needs"; const needChanges=await updateNeeds(entityId,nextTime,elapsedHours,null,active.id,active.actionType);
            phase="entity.emotions"; await applyEmotions(entityId,nextTime,needChanges,null,active.id,active.actionType);
            phase="entity.learning"; await learnFromAction(entityId,active.actionType,nextTime);
            phase="entity.development"; await updateDevelopment(sim.id,entityId,nextTime);
            phase="entity.traits"; await developTraits(entityId,nextTime,signalForDecision(active.actionType),null,active.id);
            if(["TALKING","STUDYING","WORKING","EXPLORING"].includes(active.actionType)){
              phase="entity.mental_state"; await updateMentalState(sim.id,entityId,nextTime,{currentFocus:active.actionType.toLowerCase().replaceAll("_"," "),mentalLoad:["WORKING","STUDYING"].includes(active.actionType)?0.55:0.35,certainty:0.7});
            }
            if(nextTime>=completionAt){
              ...
              await completeGoalForAction(active.metadata?.goalId||null,active.actionType,completionAt);
              await recordHabitEvidence({entityId,simulationTime:completionAt,actionType:active.actionType});
              await createMemory({simulationId:sim.id,entityId,eventId,content:`Completed ${active.actionType.toLowerCase().replaceAll("_"," ")}`,importance:0.45,strength:0.9,confidence:0.8,emotionalIntensity:0.25,simulationAt:completionAt,metadata:{perceptionSummary:perception.location||null,durationMinutes}});
            }
            phase="entity.publish"; this.hub.publish(sim.id,"entity.state",{entityId,action:{...active,status:nextTime>=completionAt?"COMPLETED":"ACTIVE",startedSimulationAt:active.startedSimulationAt,expectedCompletionSimulationAt:completionAt},needChanges});
            continue;
          }
          ...
          phase="entity.needs"; const needChanges=await updateNeeds(entityId,nextTime,0,action.eventId,action.actionId,decision.actionType);
          phase="entity.emotions"; await applyEmotions(entityId,nextTime,needChanges,action.eventId,action.actionId,decision.actionType);
          phase="entity.publish"; this.hub.publish(sim.id,"entity.state",{entityId,decision,action,status:"ACTIVE",needChanges});
        }
        phase="memory.decay"; await decayMemories(sim.id,nextTime);
        const count=(this.tickCounter.get(sim.id)||0)+1; this.tickCounter.set(sim.id,count);
