...
class GeminiService {
  constructor(){this.client=null;this.model=env.GEMINI_MODEL;this.lastAutonomyDecisionAt=new Map();this.lastRequestStatus={status:"IDLE",source:"NONE"};this.providerFailureStreak=0;}
  async init(){await budget.ensureGeminiUsageTable();if(!env.GEMINI_ENABLED||!env.GEMINI_API_KEY){logger.info("Gemini disabled or API key missing; deterministic fallback enabled");return false;}const {GoogleGenAI}=await import("@google/genai");this.client=new GoogleGenAI({apiKey:env.GEMINI_API_KEY});logger.info({model:this.model,dailyBudgetUsd:env.GEMINI_DAILY_BUDGET_USD,monthlyBudgetUsd:env.GEMINI_MONTHLY_BUDGET_USD,timeoutMs:env.GEMINI_TIMEOUT_MS,autonomyIntervalMinutes:env.GEMINI_AUTONOMY_MIN_INTERVAL_MINUTES},"Gemini cognitive budget enabled");return true;}
  canUseAutonomyDecision(entityId,simulationTime,{highValue=false}={}){if(budget.providerBlockRemainingMs()>0)return false;const previous=this.lastAutonomyDecisionAt.get(entityId);if(!previous){this.lastAutonomyDecisionAt.set(entityId,new Date(simulationTime).getTime());return true;}const elapsedMinutes=(new Date(simulationTime).getTime()-previous)/60000,configuredInterval=Number(env.GEMINI_AUTONOMY_MIN_INTERVAL_MINUTES),normalInterval=Math.max(60,Number.isFinite(configuredInterval)?configuredInterval:60),intervalMinutes=highValue?Math.min(30,normalInterval):normalInterval;if(elapsedMinutes<intervalMinutes)return false;this.lastAutonomyDecisionAt.set(entityId,new Date(simulationTime).getTime());return true;}
  async generateJson(prompt,schema,{kind="autonomy",thinkingLevel="low"}={}){
    ...
    const outputTokenCeiling=kind==="dialogue"?Number(env.GEMINI_DIALOGUE_OUTPUT_TOKEN_CEILING):Number(env.GEMINI_AUTONOMY_OUTPUT_TOKEN_CEILING),reservation=await budget.reserve({prompt,outputTokenCeiling,kind});if(!reservation.allowed){
      this.lastRequestStatus={status:"FALLBACK",source:"DETERMINISTIC_FALLBACK",reason:reservation.reason,attempted:false,retryAfterMs:Number(reservation.retryAfterMs||0),kind};
      ...
      return null;
    }const timeoutMs=Math.max(30000,Number(env.GEMINI_TIMEOUT_MS)||30000);const startedAt=Date.now();let timeoutId=null,finalized=false;const timeoutPromise=new Promise((_,reject)=>{timeoutId=setTimeout(()=>reject(Object.assign(new Error("Gemini timeout"),{code:"AI_TIMEOUT"})),timeoutMs);});const numberArray=itemProps=>({type:"array",items:{type:"object",properties:itemProps,required:Object.keys(itemProps)}});const responseSchema=schema===DialogueSchema?{type:"object",properties:{reply:{type:"string"},emotionalTone:{type:"string"},rememberedReferences:{type:"array",items:{type:"string"}},stateEffects:{type:"object",properties:{needs:numberArray({code:{type:"string"},delta:{type:"number"}}),emotions:numberArray({code:{type:"string"},delta:{type:"number"}}),traits:numberArray({code:{type:"string"},delta:{type:"number"}}),relationship:{type:"object",nullable:true,properties:{trust:{type:"number"},affection:{type:"number"},respect:{type:"number"},familiarity:{type:"number"},attraction:{type:"number"},conflict:{type:"number"},fear:{type:"number"},admiration:{type:"number"},jealousy:{type:"number"},dependence:{type:"number"},closeness:{type:"number"},irritation:{type:"number"}}},communicationStyle:{type:"object",nullable:true,properties:{formality:{type:"number"},warmth:{type:"number"},directness:{type:"number"},verbosity:{type:"number"},humor:{type:"number"},emojiUse:{type:"number"},emotionalOpenness:{type:"number"},argumentativeDepth:{type:"number"}}},goalProposal:{type:"object",nullable:true,properties:{title:{type:"string"},description:{type:"string"},priority:{type:"number"},reason:{type:"string"}}},preferences:numberArray({targetType:{type:"string"},targetEntityId:{type:"string",nullable:true},value:{type:"number"},strength:{type:"number"},confidence:{type:"number"},topic:{type:"string"}}),beliefs:numberArray({predicate:{type:"string"},subjectEntityId:{type:"string",nullable:true},objectValue:{type:"string"},confidence:{type:"number"},importance:{type:"number"}}),knowledge:numberArray({knowledgeType:{type:"string"},content:{type:"string"},subjectEntityId:{type:"string",nullable:true},objectEntityId:{type:"string",nullable:true},predicate:{type:"string",nullable:true},confidence:{type:"number"},importance:{type:"number"}}),habitCandidate:{type:"object",nullable:true,properties:{name:{type:"string"},description:{type:"string"},frequency:{type:"string"},triggerDefinition:{type:"string"},actionDefinition:{type:"string"},confidence:{type:"number"}}},reflection:{type:"object",nullable:true,properties:{thought:{type:"string",nullable:true},currentFocus:{type:"string",nullable:true},currentConcern:{type:"string",nullable:true},mentalLoad:{type:"number"},rumination:{type:"number"},certainty:{type:"number"}}},planProposal:{type:"object",nullable:true,properties:{title:{type:"string"},strategy:{type:"object"},steps:{type:"array",items:{type:"object",properties:{title:{type:"string"},description:{type:"string"},actionType:{type:"string"}},required:["title"]}}}}},required:["needs","emotions","traits","relationship","communicationStyle","goalProposal","preferences","beliefs","knowledge","habitCandidate","reflection","planProposal"]}},required:["reply","emotionalTone","rememberedReferences","stateEffects"]}:schema===DecisionSchema?{type:"object",properties:{selectedActionType:{type:"string"},targetEntityId:{type:"string",nullable:true},targetLocationId:{type:"string",nullable:true},reason:{type:"string"},confidence:{type:"number"},strategy:{type:"object",nullable:true,properties:{objective:{type:"string"},rationale:{type:"string"},constraints:{type:"array",items:{type:"string"}},fallbackActionType:{type:"string",nullable:true}}},planProposal:{type:"object",nullable:true,properties:{title:{type:"string"},strategy:{type:"object"},steps:{type:"array",items:{type:"object",properties:{title:{type:"string"},description:{type:"string"},actionType:{type:"string"}},required:["title"]}}}}},required:["selectedActionType","reason","confidence"]}:undefined;try{const response=await Promise.race([this.client.models.generateContent({model:this.model,contents:prompt,config:{responseMimeType:"application/json",responseSchema,thinkingConfig:{thinkingLevel}}}),timeoutPromise]);const raw=typeof response.text==="string"?response.text:"",parsed=schema.parse(JSON.parse(raw));await budget.finalize(reservation,response.usageMetadata);finalized=true;this.providerFailureStreak=0;this.lastRequestStatus={status:"SUCCESS",source:"GEMINI",reason:"PROVIDER_SUCCESS",attempted:true,retryAfterMs:0,kind};const usage=response.usageMetadata||{},outputTokens=Number(usage.candidatesTokenCount||0)+Number(usage.thoughtsTokenCount||0);logger.debug({kind,model:this.model,latencyMs:Date.now()-startedAt,inputTokens:Number(usage.promptTokenCount||reservation.inputTokens||0),outputTokens,selectedActionType:kind==="autonomy"?parsed.selectedActionType:undefined},"Gemini request succeeded");return parsed;}catch(err){const failure=classifyGeminiError(err),isProviderRejection=failure.kind==="RATE_LIMIT"||failure.kind==="QUOTA",fallbackReason=failure.kind==="RATE_LIMIT"?"PROVIDER_RATE_LIMIT":failure.kind==="QUOTA"?"PROVIDER_QUOTA_EXHAUSTED":failure.kind==="TIMEOUT"?"AI_TIMEOUT":"PROVIDER_ERROR",defaultCooldown=failure.kind==="QUOTA"?Number(env.GEMINI_PROVIDER_QUOTA_COOLDOWN_MS):Number(env.GEMINI_PROVIDER_RATE_LIMIT_COOLDOWN_MS),cooldown=Math.max(failure.retryAfterMs||0,defaultCooldown||0);if(!finalized){try{await budget.release(reservation);if(isProviderRejection)await budget.restoreRejectedRequest(reservation);}catch(releaseErr){logger.error({err:releaseErr,kind},"Failed to release Gemini budget reservation");}}this.lastRequestStatus={status:"FALLBACK",source:"DETERMINISTIC_FALLBACK",reason:fallbackReason,attempted:true,retryAfterMs:cooldown,kind};
      if(failure.kind==="RATE_LIMIT"||failure.kind==="QUOTA"){
        this.providerFailureStreak=0;
        budget.blockProvider(cooldown||60000,fallbackReason);
        logger.warn({kind,reason:fallbackReason,retryAfterMs:cooldown},"Gemini provider limit reached; local circuit breaker enabled");
      } else if(failure.kind==="TIMEOUT"||failure.kind==="TRANSIENT"||failure.kind==="NETWORK"){
        this.providerFailureStreak=Math.min(16,this.providerFailureStreak+1);
        const transientCooldown=computeProviderBackoffMs({
          failureStreak:this.providerFailureStreak,
          retryAfterMs:failure.retryAfterMs
        });
        const transientReason=failure.kind==="TIMEOUT"?"AI_TIMEOUT":
          failure.kind==="NETWORK"?"PROVIDER_NETWORK_FAILURE":"PROVIDER_TRANSIENT_FAILURE";
        this.lastRequestStatus.reason=transientReason;
        this.lastRequestStatus.retryAfterMs=transientCooldown;
        budget.blockProvider(transientCooldown,transientReason);
        logger.warn({
          kind,
