...
const DecisionSchema = z.object({
  ...
  strategy: z.object({
    ...
    constraints: z.array(z.string().max(200)).max(6).default([]),
    fallbackActionType: z.string().max(100).nullable().optional()
  }).nullable().optional(),
...
const DialogueSchema = z.object({
  ...
  stateEffects: z.object({
    ...
    relationship: z.object({
      trust: z.number().min(-1).max(1).optional(), affection: z.number().min(-1).max(1).optional(), respect: z.number().min(-1).max(1).optional(), familiarity: z.number().min(-1).max(1).optional(),
      attraction: z.number().min(-1).max(1).optional(), conflict: z.number().min(-1).max(1).optional(), fear: z.number().min(-1).max(1).optional(), admiration: z.number().min(-1).max(1).optional(),
      jealousy: z.number().min(-1).max(1).optional(), dependence: z.number().min(-1).max(1).optional(), closeness: z.number().min(-1).max(1).optional(), irritation: z.number().min(-1).max(1).optional()
    }).nullable().default(null),
    communicationStyle: z.object({
      formality: z.number().min(0).max(1).optional(), warmth: z.number().min(0).max(1).optional(), directness: z.number().min(0).max(1).optional(), verbosity: z.number().min(0).max(1).optional(),
      humor: z.number().min(0).max(1).optional(), emojiUse: z.number().min(0).max(1).optional(), emotionalOpenness: z.number().min(0).max(1).optional(), argumentativeDepth: z.number().min(0).max(1).optional()
    }).nullable().default(null),
    goalProposal: z.object({ title: z.string().min(1).max(120), description: z.string().max(500).optional(), priority: z.number().min(0).max(1).optional(), reason: z.string().max(300).optional() }).nullable().default(null),
    preferences: z.array(z.object({ targetType: z.string().max(50), targetEntityId: z.string().uuid().nullable().optional(), value: z.number().min(-1).max(1), strength: z.number().min(0).max(1), confidence: z.number().min(0).max(1), topic: z.string().max(80).optional() })).max(6).default([]),
    beliefs: z.array(z.object({ predicate: z.string().min(1).max(150), subjectEntityId: z.string().uuid().nullable().optional(), objectValue: z.unknown(), confidence: z.number().min(0).max(1), importance: z.number().min(0).max(1) })).max(5).default([]),
    knowledge: z.array(z.object({ knowledgeType: z.string().max(50), content: z.string().min(1).max(1000), subjectEntityId: z.string().uuid().nullable().optional(), objectEntityId: z.string().uuid().nullable().optional(), predicate: z.string().max(150).nullable().optional(), confidence: z.number().min(0).max(1), importance: z.number().min(0).max(1) })).max(5).default([]),
    habitCandidate: z.object({ name: z.string().min(1).max(150), description: z.string().max(500).optional(), frequency: z.string().max(100).optional(), triggerDefinition: z.unknown().optional(), actionDefinition: z.unknown().optional(), confidence: z.number().min(0).max(1) }).nullable().default(null),
    reflection: z.object({ thought: z.string().max(300).nullable().optional(), currentFocus: z.string().max(180).nullable().optional(), currentConcern: z.string().max(180).nullable().optional(), mentalLoad: z.number().min(0).max(1).optional(), rumination: z.number().min(0).max(1).optional(), certainty: z.number().min(0).max(1).optional() }).nullable().default(null),
    planProposal: z.object({ title: z.string().max(255), strategy: z.record(z.string(), z.unknown()).optional(), steps: z.array(z.object({ title: z.string().min(1).max(255), description: z.string().max(500).optional(), actionType: z.string().max(100).optional() })).min(1).max(8) }).nullable().default(null)
  }).default({ needs: [], emotions: [], traits: [], relationship: null, communicationStyle: null, goalProposal: null, preferences: [], beliefs: [], knowledge: [], habitCandidate: null, reflection: null, planProposal: null })
});
...
class GeminiService {
  ...
  canUseAutonomyDecision(entityId, simulationTime, { highValue = false } = {}) {
    ...
    const normalInterval = Math.max(360, Number.isFinite(configuredInterval) ? configuredInterval : 360);
    const intervalMinutes = highValue ? Math.min(120, normalInterval) : normalInterval;
  ...
  async generateJson(prompt, schema, { kind = "autonomy", thinkingLevel = "low" } = {}) {
    ...
    const outputTokenCeiling = kind === "dialogue" ? Number(env.GEMINI_DIALOGUE_OUTPUT_TOKEN_CEILING) : Number(env.GEMINI_AUTONOMY_OUTPUT_TOKEN_CEILING);
    ...
    let timeoutId = null;
    let finalized = false;
    const timeoutPromise = new Promise((_, reject) => { timeoutId = setTimeout(() => reject(Object.assign(new Error("Gemini timeout"), { code: "AI_TIMEOUT" })), env.GEMINI_TIMEOUT_MS); });
    ...
    const responseSchema = schema === DialogueSchema
      ? {
          type: "object", properties: {
            reply: { type: "string" }, emotionalTone: { type: "string" }, rememberedReferences: { type: "array", items: { type: "string" } },
            stateEffects: {
              type: "object", properties: {
                needs: numberArray({ code: { type: "string" }, delta: { type: "number" } }), emotions: numberArray({ code: { type: "string" }, delta: { type: "number" } }), traits: numberArray({ code: { type: "string" }, delta: { type: "number" } }),
      ...
      : schema === DecisionSchema
        ? {
            type:"object", properties:{
              selectedActionType:{type:"string"}, targetEntityId:{type:"string",nullable:true}, targetLocationId:{type:"string",nullable:true}, reason:{type:"string"}, confidence:{type:"number"},
              strategy:{type:"object",nullable:true,properties:{objective:{type:"string"},rationale:{type:"string"},constraints:{type:"array",items:{type:"string"}},fallbackActionType:{type:"string",nullable:true}}},
              planProposal:{type:"object",nullable:true,properties:{title:{type:"string"},strategy:{type:"object"},steps:{type:"array",items:{type:"object",properties:{title:{type:"string"},description:{type:"string"},actionType:{type:"string"}},required:["title"]}}}}
    ...
    try {
      ...
      if (!finalized) { try { await budget.release(reservation); } catch (releaseErr) { logger.error({ err:releaseErr, kind }, "Failed to release Gemini budget reservation"); } }
      const status = Number(err?.status || err?.code), rawMessage = String(err?.message || ""), retryMatch = rawMessage.match(/retryDelay[^0-9]*(\d+(?:\.\d+)?)s/i), retryMs = retryMatch ? Math.ceil(Number(retryMatch[1]) * 1000) : 60_000;
      if (status === 429 || /RESOURCE_EXHAUSTED|quota exceeded|rate.?limit/i.test(rawMessage)) { budget.blockProvider(Math.max(60_000,retryMs)); logger.warn({ kind,retryAfterMs:Math.max(60_000,retryMs) }, "Gemini provider quota/rate limit reached; local circuit breaker enabled"); }
  ...
  async dialogue(context) {
    return this.generateJson([
      ...
      "Use reflection to maintain continuity.",
      JSON.stringify(context)
    ].join("\n"), DialogueSchema, { kind:"dialogue", thinkingLevel:"medium" });
  }
}

module.exports = { GeminiService, DecisionSchema, DialogueSchema };
