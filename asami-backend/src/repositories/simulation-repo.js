...
async function buildSimulationSnapshotState(id, simulationTime) {
  const queries = [
    ...
    pool.query("SELECT BIN_TO_UUID(g.id) AS goalId,BIN_TO_UUID(g.entity_id) AS entityId,BIN_TO_UUID(g.parent_goal_id) AS parentGoalId,g.title,g.goal_type AS goalType,g.priority,g.status,g.progress,g.origin_event_id IS NOT NULL AS hasOriginEvent,g.origin_memory_id IS NOT NULL AS hasOriginMemory,g.created_simulation_at AS createdSimulationAt,COALESCE(g.completed_simulation_at,g.created_simulation_at) AS updatedSimulationAt,g.version FROM goals g JOIN entities e ON e.id=g.entity_id WHERE e.simulation_id=UUID_TO_BIN(?) AND e.status='ACTIVE' AND g.status IN ('DRAFT','ACTIVE','PAUSED') ORDER BY g.entity_id,g.priority DESC,g.created_simulation_at",[id])
  ];
