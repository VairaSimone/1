    ...
    <header className="topbar">
      <div className="topbar-context">
        <div className="status-line">
          ...
          <strong>{simulation?.status || 'NO SIMULATION'}</strong>
          <span className="muted">·</span>
          <span className="sim-time">{formatSimTime(simulation?.currentSimulationAt)}</span>
        </div>
        <div className="realtime-badge">{wsConnected ? <><Wifi size={13} /> realtime</> : <><WifiOff size={13} /> polling fallback</>}</div>
      </div>
      <div className="topbar-actions">
        <div className="speed-control"><Gauge size={14} /><span>×</span><select value={String(clockSpeed)} onChange={(e) => onSpeed(Number(e.target.value))} disabled={!simulation}>
          {[0.25, 0.5, 1, 2, 5, 10, 30, 60, 120].map((v) => <option key={v} value={v}>{v}</option>)}
        </select></div>
        {running ? <button className="icon-button" title="Pausa" onClick={onPause}><Pause size={16} /></button> : <button className="icon-button primary" title="Riprendi" onClick={onResume} disabled={!simulation || simulation.status === 'STOPPED'}><Play size={16} /></button>}
