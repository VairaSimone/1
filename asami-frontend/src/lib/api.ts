...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    const serverMessage = typeof data === 'object' && data && 'error' in data
...
export const api = {
  ...
  createSimulation: (payload: Record<string, unknown>) => request<{ simulation: Simulation; asamiEntityId: string }>('/simulations', {
  ...
  speed: (id: string, speed: number) => request<Simulation>(`/simulations/${id}/speed`, {
    method: 'POST',
