  ...
  return <>
    ...
    <div className="overview-grid">
      ...
      <Panel title="Where / what" eyebrow="WORLD STATE">
        {loc ? <div className="location-card"><div className="location-icon"><MapPin size={18} /></div><div><strong>{labelize(loc.locationType)}</strong><span>{loc.addressData ? JSON.stringify(loc.addressData) : 'Location linked to entity'}</span><small>Since {formatSimTime(loc.sinceSimulationAt)}</small></div></div> : <EmptyState icon={<MapPin size={20} />} title="No location yet" text="The current schema has no location for Asami." />}
      </Panel>
    </div>

    <Panel title="Goals" eyebrow="INTENTIONS">
      {dashboard.goals.length ? <div className="goal-table">{dashboard.goals.map((g) => <div className="goal-row" key={g.id}><div><strong>{g.title}</strong><span>{g.description || g.motivation || labelize(g.goalType)}</span></div><div className="goal-progress"><ProgressBar value={g.progress} compact /><small>{pct(g.progress)}</small></div><StatusPill value={g.status} /></div>)}</div> : <EmptyState icon={<Target size={20} />} title="No goals" text="Goals are generated by the autonomy engine from current needs." />}
    </Panel>
  </>
}
